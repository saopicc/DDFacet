...
class ClassParamMachine():
    ...
    def __init__(
        ...
        for iParm, Type in zip(xrange(self.NParam), self.SolveParam):
    ...
    def ReinitPop(self, pop, SModelArray, AlphaModel=None):
        ...
        for Type in self.SolveParam:
            ...
            for i_indiv, indiv in zip(xrange(len(pop)), pop):
                ...
                if Type == "Alpha":
                    if AlphaModel is None:
                        AlphaModel = MeanVal*np.ones(
                            (SModelArray.size,), np.float32)
                    SubArray[:] = AlphaModel[:]
                    if i_indiv != 0:
                        SubArray[:] += np.random.randn(SModelArray.size)*SigVal
    ...
    def SetSquareGrid(self, Type):
        ...
        MappingIndexToXYPix = (xx[x-x.min(), y-y.min()],
                               yy[x-x.min(), y-y.min()])
    ...
    def GiveModelArray(self, A):
        ...
        for iBand in xrange(self.NFreqBands):
            if self.MultiFreqMode:
                MA[iBand] = self.SpectralFunctionsMachine.IntExpFunc(
                    S0=S, Alpha=Alpha, iChannel=iBand, iFacet=0)
            else:
                MA[iBand] = S[:]
