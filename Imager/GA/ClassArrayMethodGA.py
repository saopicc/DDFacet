...
class ClassArrayMethodGA():
    ...
    def __init__(
     ...
     WeightFreqBands=None):
        ...
        self.PM = ClassParamMachine(
            ListPixParms, ListPixData, FreqsInfo,
            SolveParam=GD["GAClean"]["GASolvePars"])
    ...
    def SetConvMatrix(self):
        ...
        if self.ListPixData is None:
            ...
            self.ListPixData = np.array(
                [x.ravel().tolist(), y.ravel().tolist()]).T.tolist()
        if self.ListPixParms is None:
            x, y = np.mgrid[0:NPixPSF:1, 0:NPixPSF:1]
            self.ListPixParms = np.array(
                [x.ravel().tolist(), y.ravel().tolist()]).T.tolist()
        ...
        M = np.zeros(
            (self.NFreqBands,
             ...
             self.NPixListParms),
            np.float32)
        self.DirtyArray = np.zeros(
            (self.NFreqBands, 1, self.NPixListData), np.float32)
        xc = yc = NPixPSF/2
        ...
        if (self.IslandBestIndiv is not None):
            ...
            if np.max(np.abs(S)) > 0:
                AddArray = self.ToConvArray(
                    self.IslandBestIndiv, OutMode="Data")
        ...
        print>>log, "  Average M"
        self.DirtyArrayMean = np.mean(
            self.DirtyArray, axis=0).reshape(
            (1, 1, self.NPixListData))
        self.DirtyCMMean = np.mean(M, axis=0).reshape(
            (1, 1, self.NPixListData, self.NPixListParms))
        self.DirtyArrayAbsMean = np.mean(
            np.abs(self.DirtyArray),
            axis=0).reshape(
            (1, 1, self.NPixListData))
        ...
        MParms = np.zeros(
            (self.NFreqBands,
             ...
             self.NPixListParms),
            np.float32)
        self.DirtyArrayParms = np.zeros(
            (self.NFreqBands, 1, self.NPixListParms), np.float32)
        # ###############################
        # for iBand in range(self.NFreqBands):
        ...
        self.CMParms = MParms
        if self.IslandBestIndiv is not None:
            self.DirtyArrayParms += self.ToConvArray(
                self.IslandBestIndiv, OutMode="Parms")
        print>>log, "  Mean MParms"
        self.DirtyArrayParmsMean = np.mean(
            self.DirtyArrayParms, axis=0).reshape(
            (1, 1, self.NPixListParms))
        self.CMParmsMean = np.mean(MParms, axis=0).reshape(
            (1, 1, self.NPixListParms, self.NPixListParms))
    ...
    def Convolve(self, A, Norm=True, OutMode="Data"):
        ...
        for iBand in xrange(self.NFreqBands):
            ...
            ConvA[
                iBand, 0] = np.dot(
                CF, AThisBand.reshape(
                    (AThisBand.size, 1))).reshape(
                (OutSize,))
    ...
    def GiveFitness(self, individual):
        ...
        for FuncType in self.MaxFunc:
            ...
            if FuncType == "MaxFlux":
                FMax = -np.max(np.abs(Resid)
                               )/(np.sqrt(self.PixVariance)*Resid.size)
                W = self.WeightMaxFunc[FuncType]
    ...
    def testMovePix(self):
        ...
        A0 = self.PM.ModelToSquareArray(
            ArrayModel, TypeInOut=("Parms", "Parms"),
            DomainOut="Parms").copy()
    ...
    def MovePix(self, indiv, iPix, Flux, FluxWeighted=True, InReg=None):
        ...
        # ArrayModel_S.reshape((1,ArrayModel_S.size))*np.ones((2,1))
        ArrayModel_S = indiv
        A = self.PM.ModelToSquareArray(
            ArrayModel_S, TypeInOut=(
                "Parms", "Parms"), DomainOut="Parms")
        ...
        for iPix in indSel:
            ...
            if Type == 0:
                for TypeParm in self.PM.SolveParam:
                    ...
                    if TypeParm == "S":
                        ds = 0.1*np.abs(self.DirtyArrayAbsMean.ravel()
                                        [iPix]-np.abs(A[iPix]))
                    else:
                        if "Sigma" in self.PM.DicoIParm[
                                TypeParm]["Default"].keys():
                            ds = self.PM.DicoIParm[TypeParm][
                                "Default"]["Sigma"]["Value"]
                        else:
                            ds = A[iPix]
    ...
    def PlotChannel(self, pop, iGen, iChannel=0):
        ...
        IM = self.PM.ModelToSquareArray(
            ConvModelArray, TypeInOut=(
                "Data", "Data"))
        Dirty = self.PM.ModelToSquareArray(
            self.DirtyArray, TypeInOut=("Data", "Data"))
        ...
        im4 = pylab.imshow(
            IM[iChannel, 0],
            interpolation="nearest", vmin=vmin - 0.1, vmax=1.5)
        ...
        pylab.suptitle(
            'Population generation %i [%f]' %
            (iGen, best_ind.fitness.values[0]), size=16)
        # pylab.tight_layout()
        pylab.draw()
        pylab.show(False)
        pylab.pause(0.1)
