...
class ClassImToGrid():
    ...
    def __init__(self,
        ...
        self.ifzfCF = ifzfCF
        self.GD = GD
        self.FFTWMachine = ModFFTW.FFTW_2Donly_np(
    ...
    def GiveGridSharp(self, Image, DicoImager, iFacet):
        ...
        for ch in xrange(nch):
            for pol in xrange(npol):
                ...
                ModelIm[
                    ch, pol, x0p: x1p, x0p: x1p] = Image[
                    ch, pol, x0: x1, y0: y1].T[
                    :: -1, :].real
    ...
    def GiveGridFader(self, Image, DicoImager, iFacet, NormIm):
        ...
        for ch in xrange(nch):
            for pol in xrange(npol):
                ...
                ModelIm[
                    ch, pol][
                    x0p: x1p, y0p: y1p] = Image[
                    ch, pol][
                    x0d: x1d, y0d: y1d].real
                ModelIm[ch,
                        pol][x0p:x1p,
                             y0p:y1p] /= NormIm[x0d:x1d,
                                                y0d:y1d].real
     ...
     ChanSel=None):
        ...
        for ch in CSel:
            for pol in xrange(npol):
                ...
                T.timeit("4")
                ModelIm[ch, pol][
                    x0p:x1p, y0p:y1p] *= SpacialWeight[x0p:x1p, y0p:y1p]
                ...
                # return True, ModelCutOrig, ModelCutOrig_GNorm,
                # ModelCutOrig_SW, ModelCutOrig_Sphe,
                # ModelCutOrig_GNorm_SW_Sphe_CorrT
        ...
        if ToGrid:
            ...
            if SumFlux != 0:
                Grid = np.complex64(
                    self.FFTWMachine.fft(
                        np.complex64(ModelIm),
                        ChanList=CSel))
            else:
