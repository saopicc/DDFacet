...
def testGrid():
    ...
    data = GM.get(times, uvw, data, flag, (A0, A1), Grid, freqs=ChanFreq)
    data0 = -data.copy()
    ...
    def get(
        ...
        if self.GD["Compression"]["CompDeGridMode"] == 0:
            ...
            vis = _pyGridderSmear.pyDeGridderWPol(
                ...
                self.WTerm.Wplanes, np.array(
                    [self.WTerm.RefWave, self.WTerm.wmax,
                     len(self.WTerm.Wplanes),
                     self.WTerm.OverS],
                    dtype=np.float64),
                self.incr.astype(np.float64),
                freqs, [self.PolMap, FacetInfos, RowInfos],
                ParamJonesList, MapSmear, OptimisationInfos, self.LSmear, np.
                int32(ChanMapping))
