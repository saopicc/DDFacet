...
class ClassPSFServer():
    ...
    def setDicoVariablePSF(self, DicoVariablePSF, NormalisePSF=False):
        ...
        if NormalisePSF:
            ...
            for iFacet in xrange(self.NFacets):
                self.CubeMeanVariablePSF[
                    iFacet] /= np.max(self.CubeMeanVariablePSF[iFacet])
                for iChan in xrange(nch):
                    self.CubeVariablePSF[
                        iFacet, iChan] /= np.max(self.CubeVariablePSF[iFacet, iChan])

        DicoMappingDesc = {
            "freqs": DicoVariablePSF["freqs"],
    ...
    def giveFacetID(self, xp, yp):
        ...
        for iFacet in xrange(self.NFacets):
            d = np.sqrt(
                (xp - self.DicoVariablePSF[iFacet]["pixCentral"][0]) ** 2 +
                (yp - self.DicoVariablePSF[iFacet]["pixCentral"][1]) ** 2)
            if d < dmin:
                dmin = d
                ClosestFacet = iFacet
        return ClosestFacet
    ...
    def giveFacetID2(self, xp, yp):
        ...
        for iFacet in xrange(self.NFacets):
            ...
            # print "lsol, msol = ",lSol,mSol
            # #,self.DicoVariablePSF[iFacet]["pixCentral"][0],self.DicoVariablePSF[iFacet]["pixCentral"][1]
    ...
    def CalcJacobian(self):
        ...
        # import pylab
        for iFacet in xrange(self.NFacets):
            ThisCubePSF = self.CubeMeanVariablePSF[iFacet, 0, 0][
                nx/2-dx-1:nx/2+dx+1+1, nx/2-dx-1:nx/2+dx+1+1]
    ...
    def SolveOffsetLM(self, Dirty, xc0, yc0):
        ...
        xc_psf = nx_psf/2
        ThisCubePSF = self.CubeMeanVariablePSF[iFacet, 0, 0][
            xc_psf-dx:xc_psf+dx+1, xc_psf-dx:xc_psf+dx+1]
        xc, yc = xc0, yc0
        Val = Dirty[xc, yc]
    ...
    def GivePSF(self):
        return self.CubeVariablePSF[
            self.iFacet], self.CubeMeanVariablePSF[
            self.iFacet]
    ...
    def GiveFreqBandsFluxRatio(self, iFacet, Alpha):
        ...
        SumJonesChanWeightSq = self.DicoVariablePSF[
            "SumJonesChanWeightSq"][iFacet]
        ...
        # print "============"
        for iChannel in xrange(NFreqBand):
            ...
            for iMS in xrange(len(SumJonesChan)):
                ...
                ThisSumJonesChanWeightSq[
                    channels] += SumJonesChanWeightSq[iMS][ind]

            # print "==
            # ",iFacet,iChannel,np.sqrt(np.sum(np.array(ThisSumJonesChan))/np.sum(np.array(ThisSumJonesChanWeightSq)))
...
#            print "FreqArrays",len(ThisFreqs),len(BeamFactor)
            ...
            for iAlpha in xrange(NAlpha):
                ...
                FreqBandsFluxRatio[iAlpha, iChannel] = np.sqrt(np.sum(
                    BeamFactor*((ThisFreqs/RefFreq)**ThisAlpha)**2))/np.sqrt(np.sum(BeamFactorWeightSq))
                FreqBandsFluxRatio[
                    iAlpha, iChannel] /= np.sqrt(self.DicoVariablePSF["MeanJonesBand"][iFacet][iChannel])
