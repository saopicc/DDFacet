...
class ClassImageDeconvMachine():
    ...
    def __init__(self, Gain=0.3,
        ...
        if set(["Q", "U"]) < set(self.PolarizationDescriptor):
            # Luke Pratley's complex polarization CLEAN
            self.PolarizationCleanTasks.append("Q+iU")
        ...
        self._niter = np.zeros(
            [len(self.PolarizationCleanTasks)],
            dtype=np.int64)
    ...
    def SetModelRefFreq(self):
        ...
        for iChannel in xrange(self.NFreqBand):
            AllFreqs += self.DicoVariablePSF["freqs"][iChannel]
            AllFreqsMean[iChannel] = np.mean(
                self.DicoVariablePSF["freqs"][iChannel])
        # assume that the frequency variance is somewhat the same in all the
        # stokes images:
        RefFreq = np.sum(
            AllFreqsMean.ravel() *
            np.mean(
                self.DicoVariablePSF["WeightChansImages"],
                axis=1).ravel())
    ...
    def SubStep(self, xxx_todo_changeme2, LocalSM):
        ...
        if self.MultiFreqMode:  # If multiple frequencies are present construct the weighted mean
            # Get the weights (assuming they stay relatively the same over
            # stokes terms)
            W = np.mean(np.float32(self.DicoDirty["WeightChansImages"]), axis=1)
            self._MeanDirty[0,
                            :,
                            x0d:x1d,
        ...
        for pol_task_id, pol_task in enumerate(self.PolarizationCleanTasks):
            if self._niter[pol_task_id] >= self.MaxMinorIter:
                print>>log, ModColor.Str(
                    "Minor cycle CLEANing of %s has already reached the maximum number of minor "
                    "cycles... won't CLEAN this polarization further." %
                    pol_task)
            ...
            if pol_task == "I":
                ...
                raise ValueError(
                    "Invalid polarization cleaning task: %s. This is a bug" %
                    pol_task)
            ...
            # These options should probably be moved into MinorCycleConfig in
            # parset
            DoAbs = int(self.GD["ImagerDeconv"]["SearchMaxAbs"])
            print>>log, "  Running minor cycle [MinorIter = %i/%i, SearchMaxAbs = %i]" % (
                self._niter[pol_task_id], self.MaxMinorIter, DoAbs)
            ...
            x, y, MaxDirty = NpParallel.A_whereMax(
                PeakMap, NCPU=self.NCPU, DoAbs=DoAbs, Mask=self.MaskArray)
            if pol_task == "I":
                pass
            elif pol_task == "Q+iU":
            ...
            print>>log, "    Dirty %s image peak flux      = %10.6f Jy [(min, max) = (%.3g, %.3g) Jy]" % (
                pol_task, MaxDirty, mm0, mm1)
            ...
            if ThisFlux < StopFlux:
                print>>log, ModColor.Str(
                    "    Initial maximum peak %g Jy below threshold, we're done CLEANing %s" %
                    (ThisFlux, pol_task), col="green")
            ...
            try:
                for i in xrange(
                    ...
                    if ThisFlux <= StopFlux:
                        ...
                        print>>log, ModColor.Str(
                            "    CLEANing %s [iter=%i] peak of %.3g Jy lower than stopping flux" %
                            (pol_task, i, ThisFlux), col="green")
                        cont = ThisFlux > self.FluxThreshold
                        if not cont:
                            print>>log, ModColor.Str(
                                "    CLEANing %s [iter=%i] absolute flux threshold of %.3g Jy has been reached" %
                                (pol_task, i, self.FluxThreshold), col="green", Bold=True)
                    ...
                    if (i > 0) & ((i % 100) == 0):
                        ...
                        pBAR.render(
                            PercentDone, "peak %.3g i%d" %
                            (ThisFlux, self._niter[pol_task_id]))

                    nch, npol, _, _ = self._Dirty.shape
                    # Fpol contains the intensities at (x,y) per freq and
                    # polarisation
                    ...
                    if pol_task == "I":
                        ...
                        self.ModelMachine.AppendComponentToDictStacked(
                            (x, y), 1.0, np.mean(Fpol[:, indexI, 0, 0], axis=0), indexI)
                    elif pol_task == "Q+iU":
                        indexQ = self.PolarizationDescriptor.index("Q")
                        indexU = self.PolarizationDescriptor.index("U")
                        self.ModelMachine.AppendComponentToDictStacked(
                            (x, y), 1.0, np.mean(Fpol[:, indexQ, 0, 0], axis=0), indexQ)
                        self.ModelMachine.AppendComponentToDictStacked(
                            (x, y), 1.0, np.mean(Fpol[:, indexU, 0, 0], axis=0), indexU)
                    elif pol_task == "V":
                        indexV = self.PolarizationDescriptor.index("V")
                        self.ModelMachine.AppendComponentToDictStacked(
                            (x, y), 1.0, np.mean(Fpol[:, indexV, 0, 0], axis=0), indexV)
                    else:
                        raise ValueError(
                            "Invalid polarization cleaning task: %s. This is a bug" %
                            pol_task)
                    ...
                    self.SubStep(
                        (x, y), PSF*Fpol*CurrentGain*np.sqrt(JonesNorm))
                    T.timeit("SubStep")
                ...
                print>>log, ModColor.Str(
                    "    CLEANing %s [iter=%i] minor cycle interrupted with Ctrl+C, peak flux %.3g" %
                    (pol_task, self._niter[pol_task_id], ThisFlux))
                exit_msg = exit_msg + " " + "MaxIter"
                continue_deconvolution = False or continue_deconvolution
                update_model = True or update_model
                break  # stop cleaning this polarization and move on to the next one

            # Reached maximum iterations for this polarization:
            if self._niter[pol_task_id] >= self.MaxMinorIter:
                print>> log, ModColor.Str(
                    "    CLEANing %s [iter=%i] Reached maximum number of iterations, peak flux %.3g" %
                    (pol_task, self._niter[pol_task_id], ThisFlux))
                exit_msg = exit_msg + " " + "MaxIter"
                continue_deconvolution = False or continue_deconvolution
                update_model = True or update_model
