...
class ClassVariablePSF():
	...
	def givePSF(self, lcor=None, mcor=None):
       ...
       #lcor, mcor = (lcor*np.pi/180., mcor*np.pi/180.)
		...
		varietion_freq = np.pi*((self.uvd/(self.freq0))*self.dfreq*lcor + (self.uvd/(self.freq0))*self.dfreq*mcor) 
		# smearing in time
		...
		#print "******** xxx end pseudo PSF generate at cordinates (%d, %d) pixel, number pixel %d"%(lcor1,mcor1, self.npixDirty)
                #print"************************phase, smeared, ",self.phasePSF.max(), (smearPSF.real).max()
                #print"freq smearing",smearing_freq
                #print"frequency smearing",
		#pylab.imshow(smearPSF.real)
		#pylab.show()
                
		return smearPSF.real#/np.max(smearPSF.real);
...
if __name__ == "__main__":
	...
	classpsf = ClassVariablePSF(phasePSF=psf.real[0,0,...], freq0=freq0, dtime=dtime, dfreq=dfreq, cellsize=cellsize, npix=psf.shape[2])
