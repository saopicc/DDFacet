...
class ClassModelMachine():
    ...
    def setRefFreq(self, RefFreq, AllFreqs):
        ...
        # print "ModelMachine:",self.RefFreq, self.DicoSMStacked["RefFreq"],
        # self.DicoSMStacked["AllFreqs"]
    ...
    def GiveIndividual(self, ListPixParms):
        ...
        for iPix in xrange(len(ListPixParms)):
    ...
    def AppendIsland(self, ListPixParms, V, JonesNorm=None):
        ...
        for (x, y), iComp in zip(ListPix, xrange(NPixListParms)):
            if S[iComp] == 0:
                continue
    ...
    def GiveModelImage(self, FreqIn=None):
        ...
        for x, y in DicoComp.keys():
            ...
            for iSol in xrange(len(ListSols)):
    ...
    def GiveSpectralIndexMap(
        ...
        Np = 1000
        indx, indy = np.int64(np.random.rand(
            Np)*M0.shape[0]), np.int64(np.random.rand(Np)*M0.shape[1])
    ...
    def ToNPYModel(self, FitsFile, SkyModel, BeamImage=None):
        ...
        for iSource in xrange(X.shape[0]):
            x_iSource, y_iSource = X[iSource], Y[iSource]
            _, _, dec_iSource, ra_iSource = im.toworld(
                (0, 0, y_iSource, x_iSource))
