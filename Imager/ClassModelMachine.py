...
class ClassModelMachine():
    ...
    def GiveSpectralIndexMap(
            self, CellSizeRad=1., GaussPars=[(1, 1, 0)],
            DoConv=True, MaxSpi=100, MaxDR=1e+6):
        dFreq = 1e6
        ...
        if DoConv:
            M0 = ModFFTW.ConvolveGaussian(
                M0, CellSizeRad=CellSizeRad, GaussPars=GaussPars)
            M1 = ModFFTW.ConvolveGaussian(
                M1, CellSizeRad=CellSizeRad, GaussPars=GaussPars)

        # compute threshold for alpha computation by rounding DR threshold to .1
        # digits (i.e. 1.65e-6 rounds to 1.7e-6)
        ...
        if masked or mask.any():
            print>>log, ModColor.Str(
                "WARNING: some alpha pixels outside +/-%g. Masking them." %
                MaxSpi, col="red")
        return alpha
    ...
    def GiveModelList(self):
        ...
        def _model_map(coord, component):
            ...
            return map(lambda i_sol_ls: (i_sol_ls[2]["ModelType"],               # type
                                         coord,                         # coordinate
                                         i_sol_ls[1],
                                         ...
                                         i_sol_ls[2].get(
                                             "Alpha", 0.0),  # alpha
                                         i_sol_ls[2].get("ModelParams", None)),  # shape
                       map(lambda i: (i, sa[i], self.ListScales[i]), xrange(sa.size)))

        # Lazily iterate through DicoComp entries and associated ListScales and SolsArrays,
        # assigning values to arrays
        source_iter = itertools.chain.from_iterable(_model_map(
            coord, comp) for coord, comp in DicoComp.iteritems())
    ...
    def GiveModelImage(self, FreqIn=None):
        ...
        for key in DicoComp.keys():
            for pol in xrange(npol):
                # /self.DicoSMStacked[key]["SumWeights"]
                Sol = DicoComp[key]["SolsArray"][:, pol]
                x, y = key

                #print>>log, "%s : %s"%(str(key),str(Sol))

                for iFunc in xrange(Sol.size):
                    ...
                    for ch in xrange(nchan):
                        ...
                        if ThisComp["ModelType"] == "Delta":
                            ...
                            Aedge, Bedge = GiveEdges(
                                (x, y), N0, (Sup/2, Sup/2), Sup)
                            x0d, x1d, y0d, y1d = Aedge
                            x0p, x1p, y0p, y1p = Bedge

                            ModelImage[ch, pol, x0d:x1d,
                                       y0d:y1d] += Gauss[x0p:x1p, y0p:y1p]*Flux
    ...
    def CleanNegComponants(self, box=20, sig=3, RemoveNeg=True):
        print>>log, "Cleaning model dictionary from negative componants with (box, sig) = (%i, %i)" % (
            box, sig)
        ...
        if not RemoveNeg:
            ...
            Lx, Ly = np.where(
                ((ModelImage < sig * Min) & (ModelImage != 0)) | (ModelImage < 0))

        for icomp in xrange(Lx.size):
    ...
    def CleanMaskedComponants(self, MaskName):
        print>>log, "Cleaning model dictionary from masked componants using %s" % (
            MaskName)
        im = image(MaskName)
    ...
    def PutBackSubsComps(self):
        ...
        for iSource in xrange(SourceCat.shape[0]):
            ...
            if not((x1, y1) in self.DicoSMStacked["Comp"].keys()):
                ...
                self.DicoSMStacked["Comp"][
                    (x1, y1)] += ModelMachine0.DicoSMStacked["Comp"][(x0, y0)]
