...
class ClassWeighting():
    ...
    def CalcWeights(
        ...
        print>> log, "Calculating imaging weights on an [%i,%i]x%i grid with cellsize %g" % (
            npixx, npixy, nbands, cell)
        grid0 = np.zeros((nbands, npix), np.float64)
        grid = grid0.reshape((nbands*npix,))

        weights_index = [None] * len(uvw_weights_flags_freqs)

        for iMS, (uv, weights_or_path, flags, freqs) in enumerate(
                uvw_weights_flags_freqs):
            weights = NpShared.GiveArray(weights_or_path) if isinstance(
                weights_or_path, str) else weights_or_path
            if force_unity_weight:
                weights.fill(1)
                weights[flags,...] = 0
            elif weightnorm != 1:
                weights *= weightnorm
            # flip sign of negative v values -- we'll only grid the top half of
            # the plane
            uv[uv[:, 1] < 0] *= -1
            ...
            # convert to index array -- this gives the number of the uv-bin on
            # the grid
            index = y * npixx + x
            # if we're in per-band weighting mode, then adjust the index to
            # refer to each band's grid
            ...
            del uv
            print>> log, "Accumulating weights (%d/%d)" % (
                iMS + 1, len(uvw_weights_flags_freqs))
            # accumulate onto grid
            # print>>log,weights,index
            WeightingCore.accumulate_weights_onto_grid_1d(
                grid, weights.ravel(), index.ravel())
        ...
        if Weighting == "uniform":
            ...
            for band in xrange(nbands):
                print>> log, ("applying Briggs weighting (robust=%.2f, super=%.2f, band %d)" % (
                    Robust, Super, band))
                grid1 = grid0[band, :]
...
#        Lmean=3e8/np.mean(freqs)
        ...
        if Weighting == "Briggs":
            print>>log, "Weighting in Briggs mode (robust=%.1f, super=%.1f)" % (
                Robust, Super)
            print >>log, "Calculating imaging weights on an [%i,%i] grid" % (
                                                                             npix,
                                                                             npix)
            print>>log, ""
            Mode = 0
