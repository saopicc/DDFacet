...
class ClassEvolveGA():
    ...
    def __init__(
     ...
     WeightFreqBands=None):
        ...
        if ListPixData is None:
            ...
            ListPixData = np.array(
                [x.ravel().tolist(),
                 y.ravel().tolist()]).T.tolist()
        if ListPixParms is None:
            x, y = np.mgrid[0:NPixPSF:1, 0:NPixPSF:1]
            ListPixParms = np.array(
        ...
        self.ArrayMethodsMachine = ClassArrayMethodGA.ClassArrayMethodGA(
            Dirty, PSF, ListPixParms, ListPixData, FreqsInfo,
            IslandBestIndiv=IslandBestIndiv, GD=GD,
    ...
    def InitEvolutionAlgo(self):
        ...
        toolbox.register(
            "mutate",
            self.ArrayMethodsMachine.mutGaussian,
            pFlux=0.1,
            p0=0.5,
            pMove=0.1)
    ...
    def main(self, NGen=1000, NIndiv=100, DoPlot=True):
        ...
        if self.IslandBestIndiv is not None:
            if np.max(np.abs(self.IslandBestIndiv)) == 0:
                ...
                self.ArrayMethodsMachine.PM.ReinitPop(
                    self.pop, SModelArray)  # ,AlphaModel=AlphaModel)
            else:
                SModelArray = self.ArrayMethodsMachine.PM.ArrayToSubArray(
