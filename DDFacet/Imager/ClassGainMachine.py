...
class ClassGainMachine():
    ...
    def SetFluxMax(self, ThisFlux):
        if self.Mode == "Dynamic":
            x = np.abs(ThisFlux)
            ExpGain = self.GainMax * \
                np.exp(-(x/(self.rms)-self.Sigma0)/self.SigmaScale)
    ...
    def Update(self, ThisFluxMax):
        ...
        self.CurrentIter += 1
        if not((self.CurrentIter % self.IterUpdate) == 0):
            return
        ...
        if len(self.ListFlux) > LookBackStep:
            # cleaned Jy/Iteration
            MeanSlope = np.median(Slopes[-LookBackStep::])
        else:
