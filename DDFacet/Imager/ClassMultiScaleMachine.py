...
#            print>> log, "PSF box extent computed as %d" % dx
    ...
    def MakeMultiScaleCube(self, cachedscales=None):
        ...
        if self.MultiFreqMode:
            ...
            # not in multi-frequency synthesis mode. Assume no ((v-v0)/v)
            # modulation of I_model
            Alpha = np.array([0.])
        ...
        Ratios = np.float32(np.array([float(r)
                                      for r in LRatios if r != "" and r != "''"]))

        Scales = np.float32(
            np.array([float(ls) for ls in LScales if ls != "" and ls != "''"]))
...
#        print FreqBandsFluxRatio
        ...
        if cachedscales:
            ...
            for iAlpha in xrange(NAlpha):
                ...
                for iScales in xrange(ScaleStart, NScales):

                    Minor = Scales[iScales]/(2.*np.sqrt(2.*np.log(2.)))
                    Major = Minor
                    PSFGaussPars = (Major, Minor, 0.)
                    ThisPSF = ModFFTW.ConvolveGaussian(
                        ThisMFPSF.reshape(
                            (nch,
                             1,
                             nx,
                    ...
                    Gauss = ModFFTW.GiveGauss(
                        Support, CellSizeRad=1., GaussPars=PSFGaussPars)
                ...
                for iScale in xrange(ScaleStart, NScales):
                    ...
                    for ratio in Ratios:
                        for th in Theta:
                            ...
                            self.ListScales.append(
                                {"ModelType": "Gaussian", "Model": Gauss,
                                 "ModelParams": PSFGaussPars, "Scale": iScale,
                                 "Alpha": ThisAlpha})
        ...
        self.FFTMachine = ModFFTW.FFTW_2Donly_np(
            self.CubePSFScales.shape, self.CubePSFScales.dtype)
        ...
        self.GlobalWeightFunction = ModFFTW.GiveGauss(
            self.SubPSF.shape[-1],
            CellSizeRad=1., GaussPars=PSFGaussPars)
        ...
        # r0=self.WeightWidth
        # weight=(r/r0+1.)**(-1)
        self.GlobalWeightFunction = self.GlobalWeightFunction.reshape(
            (1, 1, self.SubPSF.shape[-1], self.SubPSF.shape[-1]))*np.ones((nch, npol, 1, 1), np.float32)
    ...
    def MakeBasisMatrix(self, cachedmatrix=None):
        ...
        self.WeightMeanJonesBand = self.DicoVariablePSF[
            "MeanJonesBand"][self.iFacet].reshape((nch, 1, 1, 1))
        WeightMueller = self.WeightMeanJonesBand.ravel()
        self.WeightMuellerSignal = WeightMueller * \
            self.DicoVariablePSF["WeightChansImages"].ravel()
    ...
    def GiveBasisMatrix(self, SubSubSubCoord=None):
        ...
        UVTaper = UVTaper.reshape(
            (1, 1, nx, ny))*np.ones((nch, npol, 1, 1), np.float32)
    ...
    def GiveLocalSM(self, xxx_todo_changeme, Fpol):
        ...
        if (nxs != self.DicoBasisMatrix[
                "CubePSF"].shape[-2]) | (nys != self.DicoBasisMatrix["CubePSF"].shape[-1]):
        ...
        if self.SolveMode == "MatchingPursuit":
            ...
            # print "Type %10s (sc, alpha)=(%i,
            # %f)"%(D["ModelType"],D["Scale"],D["Alpha"])
            LocalSM = self.CubePSFScales[
                indMaxSol1[0]]*MeanFluxTrue  # FpolMean.ravel()[0]
            ...
            Sol = DicoBasisMatrix[
                "BMnorm"]*np.dot(BM.T, WVecPSF*(dirtyVec/MeanFluxTrue-BM))
            # Sol=np.dot(BM.T,WVecPSF*dirtyVec)
            print x0, y0, Sol
            indMaxSol1 = np.where(np.abs(Sol) == np.max(np.abs(Sol)))[0]
            indMaxSol0 = np.where(np.abs(Sol) != np.max(np.abs(Sol)))[0]
            ...
            LocalSM = np.sum(self.CubePSFScales *
                             Sol.reshape((Sol.size, 1, 1, 1)), axis=0)
            ...
            if abs(Sol).max() < self._stall_threshold:
                ...
                print>>log, (self.iFacet, x, y, Fpol, FpolTrue, Sol, Sol0,
                             SolReg, coef, MeanFluxTrue, self.WeightMuellerSignal)
            ...
            if self.GD["Debugging"]["DumpCleanSolutions"]:
                ...
                if not debug_dump_file:
                    debug_dump_file = file(
                        self.GD["Images"]["ImageName"] + ".clean.solutions", "w")
                cPickle.dump(
                    (self.iFacet,
                     x,
                     y,
            ...
            if self.GD["Debugging"]["DumpCleanSolutions"]:
                ...
                if not debug_dump_file:
                    debug_dump_file = file(
                        self.GD["Images"]["ImageName"]+".clean.solutions", "w")
                cPickle.dump(
                    (self.iFacet, xc, yc, Fpol, FpolTrue, Sol),
                    debug_dump_file, 2)
