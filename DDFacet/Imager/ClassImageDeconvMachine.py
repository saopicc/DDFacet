...
class ClassImageDeconvMachine():
    ...
    def FindPSFExtent(self, Method="FromBox"):
        ...
        if Method == "FromBox":
            ...
            x0 = xtest[itest]
            dx0 = (x0-NPSF/2)
            print >>log, "PSF extends to [%i] from center, with rms=%.5f" % (
                                                                             dx0,
                                                                             std)
    ...
    def MakeMultiScaleCube(self):
        ...
        self.WeightFunction = ModFFTW.GiveGauss(
            self.SubPSF.shape[-1],
    ...
    def FindBestScale(self, xxx_todo_changeme, Fpol):
        ...
        chi2 = np.sum(np.sum(resid2, axis=1),
                      axis=1)/(np.sum(self.WeightFunction))
    ...
    def GiveEdges(self, xxx_todo_changeme1, N0, xxx_todo_changeme2, N1):
        (xc0, yc0) = xxx_todo_changeme1
        (xc1, yc1) = xxx_todo_changeme2
        M_xc = xc0
        M_yc = yc0
        NpixMain = N0
        F_xc = xc1
    ...
    def SubStep(self, xxx_todo_changeme3, Fpol, iScale=0):
        ...
        # pylab.draw()
        # print "Fpol02",Fpol
        NpParallel.A_add_B_prod_factor(
            (self.Dirty),
            PSF, Aedge, Bedge, factor=float(factor),
            NCPU=self.NCPU)
        # print "Fpol03",Fpol
        # pylab.subplot(1,3,3,sharex=ax,sharey=ax)
        # pylab.imshow(self.Dirty[0,x0d:x1d,y0d:y1d],interpolation="nearest",vmin=vmin,vmax=vmax)
    ...
    def Deconvolve(self, Nminor=None, ch=0):
        ...
        print>>log, "  Running minor cycle [MaxMinorIter = %i, CycleFactor=%3.1f]" % (
        ...
        print>>log, "    Dirty image peak flux   = %7.3f Jy [(min, max) = (%7.3f, %7.3f) Jy]" % (
            MaxDirty, mm0, mm1)
        print>>log, "    RMS threshold flux      = %7.3f Jy [rms      = %7.3f Jy]" % (
            FluxLimit, RMS)
        print>>log, "    Sidelobs threshold flux = %7.3f Jy [sidelobe = %7.3f of peak]" % (
        ...
        x, y, ThisFlux = NpParallel.A_whereMax(
            self.Dirty, NCPU=self.NCPU, DoAbs=1)
        # print x,y
        ...
        for i in xrange(Nminor):

            # x,y,ThisFlux=NpParallel.A_whereMax(self.Dirty,NCPU=self.NCPU,DoAbs=1)
            x, y, ThisFlux = NpParallel.A_whereMax(
                self.Dirty, NCPU=self.NCPU, DoAbs=1, Mask=self.MaskArray)
            ...
            if ThisFlux < FluxLimit:
                print>>log, "    [iter=%i] Maximum peak lower that rms-based limit of %f Jy (%i-sigma)" % (
                    i, FluxLimit, Threshold_RMS)
                return "MinFlux"

            if ThisFlux < Threshold_SideLobe:
                print>>log, "    [iter=%i] Peak residual flux %f Jy higher than sidelobe-based limit of %f Jy" % (
        ...
        print>>log, ModColor.Str(
            "    [iter=%i] Reached maximum number of iterations" %
            (Nminor))
        return "MaxIter"
