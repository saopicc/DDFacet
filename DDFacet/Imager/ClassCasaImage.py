...
class ClassCasaimage():
    ...
    def __init__(
     ...
     Stokes=["I"]):
        ...
        for si in xrange(len(self.sorted_stokes)-1):
            if self.sorted_stokes[
                    si+1] - self.sorted_stokes[si] != self.delta_stokes:
                raise RuntimeError(
                    "Your selection of Stokes parameters cannot "
        ...
        # print "image
        # refpix:",rad2hmsdms.rad2hmsdms(radec[0],Type="ra").replace(" ",":"),",
        # ",rad2hmsdms.rad2hmsdms(radec[1],Type="dec").replace(" ",".")
    ...
    def setdata(self, dataIn, CorrT=False):
        ...
        if CorrT:
            ...
            for ch in xrange(nch):
                for pol in xrange(npol):
                    # Need to place stokes data in increasing order because of
                    # the linear spacing assumption used in FITS
...
def test():
    name, imShape, Cell, radec = "lala2.psf", (10, 1, 1029, 1029), 20, (
        3.7146787856873478, 0.91111035090915093)
