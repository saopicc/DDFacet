...
def work_producer(queue, NIslands, ListIslands, Dirty,
    ...
    print>> log, "DeconvSSD Starting..."
    ListBestIndiv = []

    T = ClassTimeIt.ClassTimeIt("    ")
    T.disable()
    for iIsland in xrange(NIslands):
        ...
        JonesNorm = np.sum(
            JonesNorm * W.reshape((nchan, 1, 1, 1)),
            axis=0).reshape(
            (1, npol, 1, 1))
        T.timeit("JonesNorm")

        IslandBestIndiv = ModelMachine.GiveIndividual(ThisPixList)
        T.timeit("GiveIndividual")
        ListBestIndiv.append(IslandBestIndiv)
        FacetID = PSFServer.giveFacetID2(xm, ym)
        T.timeit("FacetID")
        ...
        try:
            ...
            queue.put(ListOrder, 60)
        except Queue.Full:
            print>> log, "work_producer: queue full"
            pass
        T.timeit("Put")

    SharedListIsland = "%s.ListIslands" % (IdSharedMem)
    ListArrayIslands = [np.array(ListIslands[iIsland])
                        for iIsland in xrange(NIslands)]
    NpShared.PackListArray(SharedListIsland, ListArrayIslands)
    T.timeit("Pack0")
    SharedBestIndiv = "%s.ListBestIndiv" % (IdSharedMem)
...
def deconv_island_worker(m_work_queue, m_result_queue, GD, IdSharedMem,
    ...
    while pill:
        try:
            ...
            iIsland, FacetID, JonesNorm = m_work_queue.get(True, 5)
        except Queue.Empty:
            ...
            if iIsland == "POISON-E":
                ...
                SharedListIsland = "%s.ListIslands" % (IdSharedMem)
                ThisPixList = NpShared.UnPackListArray(
                    SharedListIsland)[iIsland].tolist()

                SharedBestIndiv = "%s.ListBestIndiv" % (IdSharedMem)
                IslandBestIndiv = NpShared.UnPackListArray(SharedBestIndiv)[
                                                           iIsland]

                PSF = CubeVariablePSF[FacetID]
                NGen = GD["GAClean"]["NMaxGen"]
                NIndiv = GD["GAClean"]["NSourceKin"]

                ListPixParms = ThisPixList
                ListPixData = ThisPixList
                dx = GD["GAClean"]["NEnlargeData"]
                if dx > 0:
                    IncreaseIslandMachine = ClassIncreaseIsland.ClassIncreaseIsland()
                    ListPixData = IncreaseIslandMachine.IncreaseIsland(
                                                                       ListPixData,
                                                                       dx=dx)
                ...
                try:
                    ...
                    WeightMeanJonesBand = FreqsInfo["MeanJonesBand"][
                        FacetID].reshape((nch, 1, 1, 1))
                    ...
                    NpShared.ToShared(
                        "%s.FitIsland_%5.5i" %
                        (IdSharedMem, iIsland), Model)

                    # print "Current process: %s [%s
                    # left]"%(str(multiprocessing.current_process()),str(self.work_queue.qsize()))

                    m_result_queue.put({"Success": True, "iIsland": iIsland})
...
class ClassImageDeconvMachine():
    ...
    def __init__(self, Gain=0.3,
                 ...
                 ModelMachine=None, **kw    # absorb any unknown keywords arguments into this
                 ):
        # self.im=CasaImage
        ...
        self.IslandDeconvMode = self.GD["SSD"][
            "IslandDeconvMode"]  # "GA" or "Moresane" or "Sasir"
        if ModelMachine is None:
            if self.IslandDeconvMode == "GA":
                try:
                    from DDFacet.Imager.GA import ClassModelMachineGA
                    from DDFacet.Imager.GA import ClassEvolveGA
                except ImportError:
                    raise ImportError(
                        "Failed to import the Genetic Algorithm Class (ClassEvolveGA). This may be because you don't have the 'deap' package installed")
                self.ModelMachine = ClassModelMachineGA.ClassModelMachine(
                ...
                try:
                    ...
                    from DDFacet.Imager.MORESANE import ClassMoresane
                except ImportError:
                    raise ImportError(
                        "Failed to import the Genetic Algorithm Class (ClassEvolveMORESANE). This may be because you don't have the 'pymoresane' package installed")

                self.ModelMachine = ClassModelMachineMORESANE.ClassModelMachine(
                    self.GD, GainMachine=self.GainMachine)
            elif self.IslandDeconvMode == "Sasir":
                raise NotImplementedError(
                    "ClassModelMachineSASIR is not implemented")
    ...
    def SetPSF(self, DicoVariablePSF):
        ...
        DicoVariablePSF["CubeVariablePSF"] = NpShared.ToShared(
            "%s.CubeVariablePSF" %
            self.IdSharedMem, DicoVariablePSF["CubeVariablePSF"])
        self.PSFServer.setDicoVariablePSF(DicoVariablePSF)
        # self.DicoPSF=DicoPSF
        self.DicoVariablePSF = DicoVariablePSF
        # self.NChannels=self.DicoDirty["NChannels"]
        self.ModelMachine.setRefFreq(
            self.PSFServer.RefFreq,
            self.PSFServer.AllFreqs)
    ...
    def SetDirty(self, DicoDirty):
        DicoDirty["ImagData"] = NpShared.ToShared(
            "%s.Dirty.ImagData" %
            self.IdSharedMem, DicoDirty["ImagData"])
        DicoDirty["MeanImage"] = NpShared.ToShared(
            "%s.Dirty.MeanImage" %
            self.IdSharedMem, DicoDirty["MeanImage"])
    ...
    def CalcCrossIslandPSF(self, ListIslands):
        ...
        # self.PSFServer.DicoVariablePSF["MeanFacetPSF"][0,0]
        PSF = np.mean(self.PSFServer.DicoVariablePSF[
                      "MeanFacetPSF"][:, 0], axis=0)
...
# JG Mod
    def ToSquareIslands(self, ListIslands):
        ...
        for iIsland in xrange(NIslands):
            ...
            # size of the smallest square around the island
            nxisl, nyisl = maxx-minx+1, maxy-miny+1
            npix = np.max([nxisl, nyisl])

            if npix % 2 == 0:
                npix += 1

            # compute island centre from minx,maxx,miny,maxy in the
            xc, yc = np.round((maxx+minx)/2), np.round((maxy+miny)/2)

            locxc, locyc = npix/2, npix/2                         # center of the square around island

            SquareIsland = np.zeros((npix, npix), np.float32)
            SquareIsland[0:npix, 0:npix] = Dirty[
                0, 0, xc-(npix-1)/2:xc+(npix-1)/2+1, yc-(npix-1)/2:yc+(npix-1)/2+1]

            SquareIsland_Mask = np.zeros((npix, npix), np.float32)
            SquareIsland_Mask[locxc+(x-xc),
                              locyc+(y-yc)] = 1  # put 1 on the Island
            # SquareIsland[locxc+(minx-xc):locxc+(maxx-xc)+1,locyc+(miny-yc):locyc+(maxy-yc)+1]=Dirty[0,0,minx:maxx+1,miny:maxy+1]

            DicoSquareIslands[iIsland] = {
                "IslandCenter": (xc, yc),
                "IslandSquareData": SquareIsland,
                "IslandSquareMask": SquareIsland_Mask}
    ...
    def SquareIslandtoIsland(self, Model, ThisSquarePixList, ThisPixList):
        ...
        # island center in original dirty
        xc, yc = ThisSquarePixList['IslandCenter']
        # square image of the dirty around Island center
        ListSquarePix_Data = ThisSquarePixList['IslandSquareData']
        ListSquarePix_Mask = ThisSquarePixList[
            'IslandSquareMask']  # Corresponding square mask image
        ...
        for tmpcoor in ThisPixList:
            currentx = tmpcoor[0]
            currenty = tmpcoor[1]
            # coordinates in the small Model image
            x_loc_coor = (currentx - xc) + SquarePix_x / 2
            # coordinates in the small Model image
            y_loc_coor = (currenty - yc) + SquarePix_y / 2
            if ListSquarePix_Mask[
                    x_loc_coor,
                    y_loc_coor] == 1:  # if it is not masked (e.g. part of the island)
    ...
    def CalcCrossIslandFlux(self, ListIslands):
        ...
        self.CrossFluxContrib = self.PSFCross * \
            MaxIslandFlux.reshape((1, NIslands))
        ...
        print>>log, "    have grouped %i --> %i islands" % (
            NIslands, len(ListIslandMerged))
    ...
    def SearchIslands(self, Threshold):
        ...
        if dx > 0:
            ...
            IncreaseIslandMachine = ClassIncreaseIsland.ClassIncreaseIsland()
            for iIsland in xrange(len(ListIslands)):  # self.NIslands):
                ListIslands[iIsland] = IncreaseIslandMachine.IncreaseIsland(ListIslands[
                                                                            iIsland], dx=dx)
        ...
        for iIsland in xrange(len(ListIslands)):
            x, y = np.array(ListIslands[iIsland]).T
        ...
        print>>log, "  selected %i islands [out of %i] with peak flux > %.3g Jy" % (
            self.NIslands, len(ListIslands), Threshold)

        self.ListIslands_Keep = self.ListIslands

        Sz = np.array([len(self.ListIslands[iIsland])
                       for iIsland in xrange(self.NIslands)])
        ind = np.argsort(Sz)[::-1]
        ...
        # convert island to square image to pass to MORESANE & SASIR
    ...
    def DeconvolveSerial(self, ch=0):
        ...
        x, y, MaxDirty = NpParallel.A_whereMax(
            self.Dirty, NCPU=self.NCPU, DoAbs=DoAbs, Mask=self.MaskArray)
        ...
        for iIsland in xrange(self.NIslands):
            ...
            print>>log, "  Fitting island #%4.4i with %i pixels" % (
                iIsland, len(ThisPixList))
            ...
            WeightMeanJonesBand = self.FreqsInfo[
                "MeanJonesBand"][FacetID].reshape((nch, 1, 1, 1))
            WeightMueller = WeightMeanJonesBand.ravel()
            WeightMuellerSignal = WeightMueller * \
                self.FreqsInfo["WeightChansImages"].ravel()
            ...
            if self.IslandDeconvMode == "Moresane" or self.IslandDeconvMode == "Sasir":
                ...
                ListSquarePix_Center = ThisSquarePixList[
                    'IslandCenter']    # island center in original dirty
                # square image of the dirty around Island center
                ListSquarePix_Data = ThisSquarePixList['IslandSquareData']
                ListSquarePix_Mask = ThisSquarePixList[
                    'IslandSquareMask']   # Corresponding square mask image
                # size of the square postage stamp around island
                xisland, yisland = ListSquarePix_Data.shape
                ...
                if self.IslandDeconvMode == "Moresane":
                    ...
                    if xisland % 2 != 0:
                        ...
                        # PSF uneven cropped to double uneven dirty island
                        PSF2_monochan = self.PSFServer.CropPSF(
                            PSF_monochan, 2 * (xisland - 1) + 1)
                        cropped_square_to_even = True
                    else:
                        Dirty_even = ListSquarePix_Data
                        # PSF uneven cropped to double even dirty island
                        PSF2_monochan = self.PSFServer.CropPSF(
                            PSF_monochan, 2 * (xisland) + 1)

                    PSF2_monochan_nx, PSF2_monochan_ny = PSF2_monochan.shape

                    # DEAL WITH PSF IF UNEVEN (WILL ALWAYS BE UNEVEN EXCEPT IN
                    # PYMORESANE)
                    if PSF2_monochan_nx % 2 != 0:
                        PSF2_monochan_even = np.zeros(
                            (PSF2_monochan_nx-1, PSF2_monochan_nx-1))
                    ...
                    Moresane = ClassMoresane(
                        Dirty_even, PSF2_monochan_even, self.GD['MORESANE'], GD=self.GD)
                    Model_Square, Residuals = Moresane.main()

                    # 3) Apply Island mask to model to get rid of regions
                    # outside the island.
                    ...
                    NewModel, NewThisPixList = self.SquareIslandtoIsland(
                        Model_Square, ThisSquarePixList, ThisPixList)
    ...
    def DeconvolveParallel(self, ch=0):
        ...
        for cpu in xrange(n_cpus):
            p = Process(target=deconv_island_worker, args=(m_work_queue,
                                                           m_result_queue,
                                                           GD,
                                                           IdSharedMem,
                                                           FreqsInfo,
        ...
        print>>log, "Evolving %i generations of %i sourcekin" % (
            self.GD["GAClean"]["NMaxGen"], self.GD["GAClean"]["NSourceKin"])
        pBAR = ProgressBar('white', width=50, block='=', empty=' ',
                           Title=" Evolve pop.", HeaderSize=10, TitleSize=13)
        # pBAR.disable()
        pBAR.render(0, '%4i/%i' % (0, NJobs))
        ...
        while iResult < NJobs:
            ...
            if len(DicoResult) != 0 and DicoResult["Success"]:
                ...
                ThisPixList = self.ListIslands[iIsland]
                SharedIslandName = "%s.FitIsland_%5.5i" % (
                    self.IdSharedMem, iIsland)
                Model = NpShared.GiveArray(SharedIslandName)
                self.ModelMachine.AppendIsland(ThisPixList, Model)
                NpShared.DelArray(SharedIslandName)
            elif DicoResult["Success"] == False:
                print>> log, "Deconv: Returned result False"
    ...
    def SubStep(self, xxx_todo_changeme2, LocalSM):
        ...
        self._MeanDirty[0,
                        :,
                        x0d:x1d,
                        y0d:y1d] -= np.sum(LocalSM[:,
                                                   :,
                                                   x0p:x1p,
                                                   y0p:y1p]*W.reshape((W.size,
                                                                       1,
                                                                       1,
                                                                       1)),
                                           axis=0)
...
class WorkerDeconvIsland(multiprocessing.Process):
    ...
    def __init__(self,
        ...
        self.IdSharedMem = IdSharedMem
        self.FreqsInfo = FreqsInfo
        self.CubeVariablePSF = NpShared.GiveArray(
            "%s.CubeVariablePSF" % self.IdSharedMem)
    ...
    def run(self):
        ...
        while not self.kill_received:
            # gc.enable()
            try:
                iIsland, FacetID, JonesNorm = self.work_queue.get(True, 0.5)
            except:
                break
            else:
                # iIsland=DicoOrder["iIsland"]
                ...
                try:
                    ...
                    WeightMueller = WeightMeanJonesBand.ravel()
                    WeightMuellerSignal = np.sqrt(
                        WeightMueller*self.FreqsInfo["WeightChansImages"].ravel())
                    ...
                    NpShared.ToShared(
                        "%s.FitIsland_%5.5i" %
                        (self.IdSharedMem, iIsland), Model)
