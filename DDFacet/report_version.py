...
def report_version():
    ...
    try:
        ...
        result = str(subprocess.check_output('cd %s; git describe --tags' % path, shell=True, stderr=subprocess.STDOUT).rstrip())
    ...
    if result is not None and 'fatal' not in result:
        ...
        try:
            result = str(subprocess.check_output('cd %s; git rev-parse --short HEAD' % path, shell=True, stderr=subprocess.STDOUT).rstrip())
        except subprocess.CalledProcessError:
            result = None
        if result is not None and 'fatal' not in result:
