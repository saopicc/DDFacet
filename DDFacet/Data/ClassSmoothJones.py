...
class ClassSmoothJones():
    ...
    def FindAlpha(self):
        ...
        l_List = DicoJonesMatrices["DicoJones_killMS"][
            "DicoClusterDirs"]["l"].tolist()
        m_List = DicoJonesMatrices["DicoJones_killMS"][
            "DicoClusterDirs"]["m"].tolist()
        ...
        for (iDir, l, m) in zip(xrange(NDir), l_List, m_List):
            self.AlphaReg[
                iDir, :] = self.FindAlphaSingleDir(
                DicoJonesMatrices, l, m)
    ...
    def SmoothJones(self):
        ...
        for idir_kMS in xrange(NDir):
            for iAnt in xrange(na):
                alpha = self.AlphaReg[idir_kMS, iAnt]
                Jm[:, idir_kMS, iAnt, :, :, :] = Jm[:, idir_kMS, iAnt, :,
                                                    :, :]*alpha+(1.-alpha)*J0[:, idir_kMS, iAnt, :, :, :]
    ...
    def FindAlphaSingleDir(self, DicoJonesMatrices, l0, m0):
        ...
        if Apply_killMS:
            ...
            JonesMatrices_killMS = (np.abs(JonesMatrices_killMS[
                                    :, idir_kMS, :, :, 0, 0])+np.abs(JonesMatrices_killMS[:, idir_kMS, :, :, 1, 1]))/2.
