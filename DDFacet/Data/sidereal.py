...
def parseAngle(s):
    ...
    if len(minTail) != 0:
        ...
        #   else -> raise SyntaxError ]
        if len(secTail) != 0:
            second, checkTail = parseFloatSuffix(
                secTail, S_PAT, "Seconds followed by 's'")
            if len(checkTail) != 0:
...
# - - -   M i x e d U n i t s . m i x T o S i n g l e
    ...
    def mixToSingle(self, coeffs):
        ...
        #        ... ]
        for i in xrange(-1, -len(self.factors)-1, -1):
            total += coeffList[i]
...
# - - -   M i x e d U n i t s . s i n g l e T o M i x
    ...
    def singleToMix(self, value):
        ...
        for factorx in xrange(len(self.factors)):
