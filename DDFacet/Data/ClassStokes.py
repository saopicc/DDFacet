...
class ClassStokes:
    ...
    def __init__(self, MSDataDescriptor, FITSPolDescriptor):
        ...
        # Check if the correlations specified in the MS are supported by the
        # conversion code:
        for corrId, corr in enumerate(self._MSDataDescriptor):
            if corr not in StokesTypes.values():
                raise ValueError(
                    "Measurement Set contains unsupported correlation specifier %s" %
                    corr)
        ...
        # Check if the requested FITS polarizations are supported by the
        # converter:
        self._FITSstokesList = []
        if isinstance(FITSPolDescriptor, list):
            for corr in FITSPolDescriptor:
                if corr not in StokesTypes:
                    raise ValueError(
                        "Polarization %s invalid must be one of %s" %
                        (corr, StokesTypes.keys()))
                self._FITSstokesList.append(corr)
        elif isinstance(FITSPolDescriptor, str):
            if not re.match(r"^[IQUV]{1,4}$", FITSPolDescriptor.strip()):
                raise ValueError(
                    "Polarization stokes string invalid must be one or more of IQUV")
            for stokes in FITSPolDescriptor.strip():
                self._FITSstokesList.append(stokes)
        else:
            raise ValueError(
                "Image polarization descriptor must be stokes string or string list")
    ...
    def _extractStokesCombinationExpression(exp):
        ...
        if not isinstance(exp, list) or len(exp) < 2:
            raise ValueError(
                "Expected stokes dependency of the form x0 [x1 ... xN] expr")
        deps = exp[0:len(exp)-1]
        combExp = exp[len(exp)-1]
        vals = re.match(
            r"(?P<scalar>[-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[0-9]+)?)?(?P<imag>i)?"
            r"\((?P<firstIdImag>i)?(?P<firstId>[0-9])(?:(?P<op>[+-])(?P<secondIdImag>i)?(?P<secondId>[0-9]))?\)",
            combExp)

        if vals is None:
            raise ValueError(
                "Expression for combination of stokes parameters malformed")
        ...
        secondId = 1 if vals.group("secondId") is None else exp[
                                   int(vals.group("secondId"))]
    ...
    def corrs2stokes(self, corrCube):
        ...
        # Check that the Measurement Set has the required correlation data to
        # form the requested polarization products:
        _stokesExpr = []
        for stokesId, stokes in enumerate(self._FITSstokesList):
            ...
            for dep_i, dep in enumerate(depOptions):
                ...
                if False not in [(corr in self._MSDataDescriptor)
                                 for corr in depIndicies]:
                    satisfied = True
                    _stokesExpr.append(dep_i)
                    break
            if not satisfied:
                raise ValueError(
                    "Required data for stokes term %s not "
                    "available in MS. Verify your parset options." %
                    stokes)

        if not np.iscomplexobj(corrCube):
            raise TypeError(
                "Correlation cube must be of type complex (certain stokes terms cannot be reconstructed from real data)")
        ...
        for stokesId, (stokes, depExprId) in enumerate(
                zip(self._FITSstokesList, _stokesExpr)):
            ops = self._extractStokesCombinationExpression(
                StokesDependencies[stokes][depExprId])
            stokesCube[:,
                       stokesId,
                       :,
                       :] = (ops[0]*ops[1])*ops[3](ops[5] * corrCube[:,
                                                                     self._gridMSCorrMapping[ops[2]],
                                                                     :,
                                                                     :],
                                                   ops[6] * corrCube[:,
                                                                     self._gridMSCorrMapping[ops[4]],
                                                                     :,
                                                                     :])
        return stokesCube
    ...
    def stokes2corrs(self, stokesCube):
        ...
        for corrId, corr in enumerate(self._MScorrLabels):
            ...
            for dep_i, dep in enumerate(depOptions):
                ...
                if False not in [
                        (stokePar in
                         [StokesTypes[s] for s in self._FITSstokesList])
            ...
            if not satisfied:
                raise ValueError(
                    "Required data for computing correlation term %s not "
                    "available in requested imaging Stokes products. Verify your parset options." %
                    corr)

        if not np.iscomplexobj(stokesCube):
            raise TypeError(
                "Stokes cube must be of type complex (certain correlation terms cannot be reconstructed from real data)")
        ...
        for corrId, (corr, depExprId) in enumerate(
                zip(self._MScorrLabels, _corrsExpr)):
            ops = self._extractStokesCombinationExpression(
                CorrelationDependencies[corr][depExprId])
            corrCube[:,
                     corrId,
                     :,
                     :] = (ops[0]*ops[1])*ops[3](ops[5] * stokesCube[:,
                                                                     self._FITSstokesSliceLookup[ops[2]],
                                                                     :,
                                                                     :],
                                                 ops[6] * stokesCube[:,
                                                                     self._FITSstokesSliceLookup[ops[4]],
                                                                     :,
                                                                     :])
