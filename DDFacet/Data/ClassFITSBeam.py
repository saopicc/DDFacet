...
class ClassFITSBeam (object):
    ...
    def __init__(self, ms, opts):
        ...
        self.pos0 = dm.position('itrf', *
                                [dq.quantity(x, 'm')
                                 for x in self.ms.StationPos[0]])

        # make direction measure from field centre
        self.field_centre = dm.direction(
            'J2000', dq.quantity(
                self.ms.rarad, "rad"), dq.quantity(
                self.ms.decrad, "rad"))
        ...
        if feed:
            if len(feed) != 2:
                raise ValueError(
                    "FITSFeed parameter must be two characters (e.g. 'xy')")
            feed = feed.lower()
            CORRS = [a+b for a in feed for b in feed]
            print>>log, "polarization basis specified by FITSFeed parameter: %s" % " ".join(
                CORRS)
        else:
            # NB: need to check correlation names better. This assumes four
            # correlations in that order!
            if "x" in self.ms.CorrelationNames[0].lower():
                CORRS = "xx", "xy", "yx", "yy"
                print>>log, "polarization basis is linear (MS corrs: %s)" % " ".join(
                    self.ms.CorrelationNames)
            else:
                CORRS = "rr", "rl", "lr", "ll"
                print>>log, "polarization basis is circular (MS corrs: %s)" % " ".join(
                    self.ms.CorrelationNames)
        # Following code is nicked from Cattery/Siamese/OMS/pybeams_fits.py
        REIM = "re", "im"
        REALIMAG = dict(re="real", im="imag")

        # get the Cattery: if an explicit path to Cattery set, use this and
        # import Siamese directly
        ...
        if explicit_cattery:
            ...
            print>>log, "explicit Cattery path set: using custom Siamese module from %s" % os.path.dirname(
                Siamese.__file__)
        else:
            ...
            print>>log, "using standard Cattery.Siamese module from %s" % os.path.dirname(
                Siamese.__file__)
        ...
        def make_beam_filename(filename_pattern, corr, reim):
            ...
            return Utils.substitute_pattern(
                filename_pattern, corr=corr.lower(),
                xy=corr.lower(),
                CORR=corr.upper(),
                XY=corr.upper(),
                reim=reim.lower(),
                REIM=reim.upper(),
                ReIm=reim.title(),
                realimag=REALIMAG[reim].lower(),
                REALIMAG=REALIMAG[reim].upper(),
        ...
        for reFits, imFits in zip(filename_real, filename_imag):
            print>>log, "Loading beam patterns %s, %s" % (
                list(filename_real), list(filename_imag))
    ...
    def getBeamSampleTimes(self, times):
        ...
        for t in times[1:]:
            if t - beam_times[-1] >= dt:
                beam_times.append(t)
        print>>log, "  DtBeamMin=%.2f min results in %d samples" % (
            self.time_inc, len(beam_times))
        if self.pa_inc:
            pas = [
                # put antenna0 position as reference frame. NB: in the future
                # may want to do it per antenna
            ...
            for t, pa in zip(beam_times[1:], pas[1:]):
                if abs(pa-pa0) >= self.pa_inc:
                    ...
                    pa0 = pa
            print>>log, "  FITSParAngleIncrement=%.2f deg results in %d samples" % (
                self.pa_inc, len(beam_times1))
    ...
    def getFreqDomains(self):
        ...
        df = (self.freqs[1]-self.freqs[0]
              )/2 if len(self.freqs) > 1 else self.freqs[0]
    ...
    def evaluateBeam(self, t0, ra, dec):
        ...
        # put antenna0 position as reference frame. NB: in the future may want
        # to do it per antenna
        ...
        beamjones = [
            self.vbs[i].interpolate(l, m, freq=self.freqs, freqaxis=1)
            for i in xrange(4)]

        # now make output matrix
        jones = numpy.zeros(
            (ndir, self.ms.na, len(
                self.freqs), 2, 2), dtype=numpy.complex64)
        ...
        for iant in xrange(self.ms.na):
            for ijones, (ix, iy) in enumerate(((0, 0), (0, 1), (1, 0), (1, 1))):
                bj = beamjones[ijones]
                jones[:, iant, :, ix, iy] = beamjones[ijones].reshape(
                    (len(bj), 1)) if bj.ndim == 1 else bj
        return jones
