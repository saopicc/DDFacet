...
static PyMethodDef _pyGridderSmearPolsFaster_testMethods[] = {
...
void init_pyGridderSmearPolsFaster()  {
  (void) Py_InitModule("_pyGridderSmearPolsFaster", _pyGridderSmearPolsFaster_testMethods);
...
void gridderWPol(PyArrayObject *grid,
		 ...
		 )
  {
    ...
    float complex *VisBuffer=calloc(1,(4)*sizeof(float complex));
    int ThisPol;
	...
	if((DoApplyJones)&(inx==0)){
	  updateJones(irow, chStart, uvwPtr, 1, 1);
	  Zero(VisBuffer);
	} //endif DoApplyJones
	...
	for (visChan=chStart; visChan<chEnd; ++visChan) {
	  ...
	  if(DoApplyJones){
	    ...
	    int CondLastVisOfBlock=((visChan==(chEnd-1))&(inx==(NRowThisBlock-1)));
	    int CondStack=((CondLastVisOfBlock)|(SameJonesAsCurrent(irow, visChan)==0));
	    //printf("%i",CondStack);
	    if(CondStack){
	      Mat_A_Bl_Sum(VisBuffer,SkyType,VisMeas,SkyType,Weight);
	      MatDot(J0H,JonesType,VisBuffer,SkyType,VisBuffer);
	      MatDot(VisBuffer,SkyType,J1,JonesType,VisBuffer);
	      Mat_A_Bl_Sum(Vis,SkyType,VisBuffer,SkyType,1.);
	      updateJones(irow, visChan, uvwPtr, 1, 1);
	      Zero(VisBuffer);
	    }else{
	      Mat_A_Bl_Sum(VisBuffer,SkyType,VisMeas,SkyType,Weight);
...
void DeGridderWPol(PyArrayObject *grid,
		 ...
		 PyArrayObject *np_ChanMapping)
  {
    ...
    float complex *visBuff=calloc(1,4*sizeof(float complex));
    float complex *visBuffUnCorr=calloc(1,4*sizeof(float complex));
    sem_t * Sem_mutex;
    ////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
	...
	//printf("  row=[%i] %i>%i \n",irow,chStart,chEnd);
	//if(DoApplyJones){
	//  updateJones(irow, chStart, uvwPtr, 0,0);
	//} //endif DoApplyJones
	...
	for (visChan=chStart; visChan<chEnd; ++visChan) {
	  ...
	  if(DoApplyJones){
	    int CondFirst=((visChan==chStart)|(inx==0));
	    int CondApplyJones=((CondFirst)|(SameJonesAsCurrent(irow, visChan)==0));
	    Zero(visBuff);
	    if(CondApplyJones){
	      updateJones(irow, visChan, uvwPtr, 0, 0);
	      Zero(visBuffUnCorr);
	      MatDot(J0,JonesType,ThisVis,SkyType,visBuffUnCorr);
	      MatDot(visBuffUnCorr,SkyType,J1H,JonesType,visBuffUnCorr);
	    }
	    //printMat(visBuffUnCorr);
	    Mat_A_Bl_Sum(visBuff, SkyType, visBuffUnCorr, SkyType, corr);
	    //printMat(visBuff);
	  }else{
	    for(ThisPol =0; ThisPol<4;ThisPol++){
