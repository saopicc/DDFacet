...
def test():
    ...
    test_array[11, 11] = 1
    # test_array.fill(1)
    # test_array[size*3/8:size*5/8, size*3/8:size*5/8] = 1+1j # square
    # aperture oversampling 2...
    A = test_array
    F = FFTWnp(A)
...
def test2():
    ...
    dtype = np.complex128
    test_array = np.zeros((size, size), dtype=dtype)
    # square aperture oversampling 2...
    test_array[size*3/8:size*5/8, size*3/8:size*5/8] = 1+1j
    A = test_array
    for i in xrange(5):
        print i
        l.append(FFTW(A))
...
class FFTW():
    ...
    def __init__(self, A, ncores=1):
        ...
        self.A = pyfftw.interfaces.numpy_fft.fft2(
            self.A, axes=(-1, -2),
            overwrite_input=True, planner_effort='FFTW_MEASURE',
            threads=self.ncores)
        T.timeit("planF")
        self.A = pyfftw.interfaces.numpy_fft.ifft2(
            self.A, axes=(-1, -2),
            overwrite_input=True, planner_effort='FFTW_MEASURE',
            threads=self.ncores)
        T.timeit("planB")
    ...
    def fft(self, Ain):
        ...
        for ich in xrange(nch):
            for ipol in xrange(npol):
                self.A[
                    :, :] = iFs(
                    A[ich, ipol].astype(self.ThisType),
                    axes=axes)
                T.timeit("shift and copy")
                self.A = pyfftw.interfaces.numpy_fft.fft2(
                    self.A, axes=(-1, -2),
...
def ConvolveGaussian(
    Ain0, CellSizeRad=None, GaussPars=[
        (0., 0., 0.)], Normalise=False):
