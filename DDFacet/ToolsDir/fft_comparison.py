...
def fft_comparison_tests(size=2048, dtype=np.complex128, byte_align=False):
    ...
    test_array[size*3/8:size*5/8, size*3/8:size*5/8] = 1
    ...
    for FFT_direction in ['forward']:  # ,'backward']:
        ...
        print ""
        # Let's first deal with some planning to make sure wisdom is generated
        # ahead of time.
        for i, fft_type in enumerate(['numpy', 'pyfftw3', 'pyfftw']):
            ...
            if fft_type == 'numpy':
                ...
                if byte_align:
                    test_array = pyfftw.n_byte_align_empty(
                        (size, size), 16, dtype=dtype)
                test_array = pyfftw.interfaces.numpy_fft.fft2(
                    test_array, overwrite_input=True,
                    planner_effort='FFTW_MEASURE', threads=ncores)
        ...
        for i, fft_type in enumerate(['numpy', 'pyfftw3', 'pyfftw']):
            ...
            norm = matplotlib.colors.LogNorm(
                vmin=psf.max()*1e-6, vmax=psf.max())
