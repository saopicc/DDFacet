...
class ClassSpectralFunctions():
    ...
    def setFreqs(self):
        ...
        for iChannel in xrange(self.NFreqBand):
            ...
            AllFreqsMean[iChannel] = np.mean(
                self.DicoMappingDesc["freqs"][iChannel])

        RefFreq = np.sum(AllFreqsMean.ravel() *
                         self.DicoMappingDesc["WeightChansImages"].ravel())
    ...
    def GiveBeamFactorsFacet(self, iFacet):
        ...
        for iChannel in xrange(self.NFreqBand):
            ...
            for iMS in xrange(len(SumJonesChan)):
                ...
                ThisSumJonesChanWeightSq += SumJonesChanWeightSq[
                    iMS][ind].tolist()
    ...
    def IntExpFunc(self, S0=1., Alpha=0., iChannel=0, iFacet=0):
        ...
        if self.BeamEnable:
            ListBeamFactor, ListBeamFactorWeightSq = self.DicoBeamFactors[iFacet][
                "ListBeamFactor"], self.DicoBeamFactors[iFacet]["ListBeamFactorWeightSq"]
            BeamFactor = ListBeamFactor[iChannel].reshape((1, ThisFreqs.size))
            BeamFactorWeightSq = ListBeamFactorWeightSq[
                iChannel].reshape((1, ThisFreqs.size))
            MeanJonesBand = self.DicoMappingDesc[
                "MeanJonesBand"][iFacet][iChannel]
        else:
            BeamFactor = 1.
        ...
        FreqBandsFlux = np.sqrt(np.sum(
            BeamFactor*((ThisFreqs/RefFreq)**ThisAlpha)**2, axis=1))/np.sqrt(np.sum(BeamFactorWeightSq))
