...
class ClassCoordConv():
    ...
    def __init__(self, rac, decc):
        ...
        mrot = np.matrix(
            [[cos(rarad) * cos(decrad),
             ...
             [-sin(rarad),
              cos(rarad),
              0.],
             [-cos(rarad) * sin(decrad),
              -sin(rarad) * sin(decrad),
              cos(decrad)]]).T
        ...
        self.vn2 = mrot*vn
        self.R = np.matrix(
            [[cos(decrad)*cos(rarad), cos(decrad)*sin(rarad), sin(decrad)]]).T
    ...
    def lm2radec(self, l_list, m_list):
        ...
        for i in xrange(l_list.shape[0]):
            l = l_list[i]
            m = m_list[i]
            if (l_list[i] == 0.) & (m_list[i] == 0.):
                ...
                continue
            Rp = self.R+self.vl2*l+self.vm2*m - \
                (1.-np.sqrt(1.-l**2-m**2))*self.vn2
