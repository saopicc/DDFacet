...
def GiveMDC(ParsetFile="ParsetNew.txt",
    ...
    if "FileSourceCat" in GD.DicoConfig["Files"].keys():
        ...
        ListSM = [None for i in xrange(NPointing)]
        ThereIsSM = False

    for MSname, SMname in zip(ListMS, ListSM):
        MS0 = ClassMS.ClassMS(MSname, Col=GD.DicoConfig["Files"][
                              "ColName"], DoReadData=DoReadData)
        if ThereIsSM:
