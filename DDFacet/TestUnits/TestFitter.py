...
def testRestore():
    def restoreFittedBeam(rot):
        ...
        inp = gauss2d(
            [1, imgSize / 2, imgSize / 2, params[1],
             params[0],
             params[2]],
        ...
        # fit
        fittedParams = tuple((fitter.FitCleanBeam(
            inp[0, 0, :, :]) * np.array([cellSize, cellSize, 1])).tolist())
        # restore fitted clean beam with an FFT convolution:
        delta = np.zeros([1, 1, imgSize, imgSize])
        delta[0, 0, imgSize / 2, imgSize / 2] = 1
        rest = fftconvolve.ConvolveGaussian(
            delta, cellSize, GaussPars=[fittedParams], Normalise=False)
...
def testFitSinc():
    ...
    # just check if it found the first sidelobe level
    assert np.isclose(lev, np.sinc(5/2.0), rtol=1e-2, atol=1e-8)
