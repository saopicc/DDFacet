...
class ClassMS():
    ...
    def LoadLOFAR_ANTENNA_FIELD(self):
        ...
        try:
            ...
            TileOffXYZ = t.getcol("TILE_ELEMENT_OFFSET").reshape(
                na, 1, nAntPerTiles, 3)
    ...
    def GiveMappingAnt(
        ...
        (row0, row1) = xxx_todo_changeme
        if not isinstance(ListStrSel, list):
            assert(False)

        #ListStrSel=["RT9-RTA", "RTA-RTB", "RTC-RTD", "RT6-RT7", "RT5"]

        print>>log, ModColor.Str(
            "  ... Building BL-mapping for %s" %
            str(ListStrSel))
        ...
        def GiveStrAntToNum(self, StrAnt):
            ind = []
            for i in xrange(len(self.StationNames)):
            ...
            if "-" in blsel:
                ...
                for NumStrA0 in LNumStrA0:
                    for NumStrA1 in LNumStrA1:
                        NumA0 = np.where(
                            np.array(
                                self.StationNames) == NumStrA0)[0]
                        NumA1 = np.where(
                            np.array(
                                self.StationNames) == NumStrA1)[0]
    ...
    def getChunkCache(self, row0, row1):
        if (row0, row1) not in self._chunk_caches:
            self._chunk_caches[
                row0, row1] = CacheManager(
                self.MSName + ".ddfcache/F%d:D%d:%d:%d" %
                (self.Field, self.DDID, row0, row1),
                self._reset_cache)
        return self._chunk_caches[row0, row1]

    def GiveNextChunk(self, databuf=None, flagbuf=None):
        # get row0:row1 of next chunk. If row1==row0, chunk is empty and we must
        # skip it
        while self.current_chunk < self.Nchunk-1:
            ...
            if row1 > row0:
                ...
                return self.ReadData(
                    row0, row1, databuf=databuf, flagbuf=flagbuf)
        return "EndMS"
    ...
    def ReadData(
     ...
     flagbuf=None):
        ...
        # [SPW==self.ListSPW[0]]
        A0 = table_all.getcol('ANTENNA1', row0, nRowRead)
        # [SPW==self.ListSPW[0]]
        A1 = table_all.getcol('ANTENNA2', row0, nRowRead)
        ...
        if flagbuf is not None:
            flag_all = np.ndarray(
                ...
                dtype=np.bool,
                buffer=flagbuf)
            print>>log, "using %d/%d elements of existing flag buffer" % (
                flag_all.size, flagbuf.size)
            table_all.getcolslicenp(
                "FLAG",
                flag_all,
                self.cs_tlc,
                self.cs_brc,
            ...
            flag_all = table_all.getcolslice(
                "FLAG",
                self.cs_tlc,
                self.cs_brc,
                self.cs_inc,
        ...
        if databuf is not None:
            vis_all = np.ndarray(
                ...
                dtype=np.complex64,
                buffer=databuf)
            print>>log, "using %d/%d elements of existing visibility buffer" % (
                vis_all.size, databuf.size)
            table_all.getcolslicenp(
                self.ColName,
                vis_all,
                self.cs_tlc,
        ...
        if self.zero_flag:
            vis_all[flag_all == 1] = 1e10
        # print
        # "count",np.count_nonzero(flag_all),np.count_nonzero(np.isnan(vis_all))
    ...
    def GiveAverageTimeFreq(self, DicoData, StepTime=None, StepFreq=None):
        ...
        VisOut = np.zeros(
            (NTimesOut, self.nbl, NChanOut, 4),
            dtype=DicoData["data"].dtype)
        FlagOut = np.ones(
            (NTimesOut, self.nbl, NChanOut, 4),
            dtype=DicoData["flag"].dtype)
        NPointsOutChan = np.zeros(
            (NTimesOut, self.nbl, NChanOut, 4),
            dtype=np.int32)
        ...
        for itime in xrange(NTimesOut):
            ibl = 0
            for iA0 in xrange(self.na):
                for iA1 in xrange(iA0, self.na):
                    A0Out[itime, ibl] = iA0
                    A1Out[itime, ibl] = iA1
                    blNum[iA0, iA1] = ibl
                    ibl += 1
    ...
    def SaveAllDataStruct(self):
        ...
        for icol in xrange(len(self.ColName)):
    ...
    def ReadMSInfo(self, DoPrint=True):
        ...
        if not table_all.nrows():
            raise RuntimeError(
                "no rows in MS %s, check your Field/DDID/TaQL settings" %
                (self.MSName))
        ...
        self.CorrelationNames = [
            (ctype >= 0 and ctype <
             len(MS_STOKES_ENUMS) and MS_STOKES_ENUMS[ctype]) or None
        ...
        if not self.TimeChunkSize:
            ...
            print>>log, "ChunkHours is not set; MS %s (%d rows) will be processed as a single chunk" % (
                self.MSName, self.F_nrows)
            chunk_rows = [0]
        else:
            all_times = table_all.getcol("TIME")
            if (all_times[1:] - all_times[:-1]).min() < 0:
                raise RuntimeError(
                    "MS %s: the TIME column must be in increasing order" %
                    self.MSName)
            ...
            if len(chunk_row0) == 1:
                print>>log, "MS %s (%d rows) will be processed as a single chunk" % (
                    self.MSName, self.F_nrows)
            else:
                print>>log, "MS %s (%d rows) will be split into %d chunks, at rows %s" % (
                    self.MSName, self.F_nrows, len(chunk_row0), " ".join(map(str, chunk_row0)))
        self.Nchunk = len(chunk_row0)
        chunk_row0.append(self.F_nrows)
        self._chunk_r0r1 = [chunk_row0[i:i+2] for i in xrange(self.Nchunk)]
        ...
        self.dFreq = ta_spectral.getcol("CHAN_WIDTH")[
            self._spwid, self.ChanSlice].flatten()[0]
        self.ChanWidth = ta_spectral.getcol(
            'CHAN_WIDTH')[self._spwid, self.ChanSlice]
        ...
        if Nchan > 1:
            self.DoRevertChans = (self.ChanFreq.flatten()[
                                  0] > self.ChanFreq.flatten()[-1])
    ...
    def __str__(self):
        ...
        ll.append("   - Selection: %s, channels: %s" %
                  (ModColor.Str(str(self.TaQL), col="green"), self.ChanSlice))
        ll.append(
            "   - Phase centre (field %d): (ra, dec)=(%s, %s) " %
            (self.Field,
             rad2hmsdms(
                 self.rarad,
                 Type="ra").replace(
    ...
    def SaveVis(self, vis=None, Col="CORRECTED_DATA", spw=0, DoPrint=True):
        if vis is None:
            vis = self.data
        if DoPrint:
            print>>log, "Writting data in column %s" % ModColor.Str(
                Col, col="green")
    ...
    def GiveVisBLChan(self, a0, a1, chan, pol=None):
        if pol is None:
            vecout = (
                self.data[(self.A0 == a0) & (self.A1 == a1),
                          chan, 0] + self.data
    ...
    def CopyCol(self, Colin, Colout):
        ...
        if self.TimeChunkSize is None:
            ...
            if not(self.DTh in TimesInt):
                TimesInt.append(self.DTh)
            for i in xrange(len(TimesInt)-1):
    ...
    def AddCol(self, ColName, LikeCol="DATA", quiet=False):
        t = table(self.MSName, readonly=False, ack=False)
        if (ColName in t.colnames() and not self.GD[
                "Images"]["AllowColumnOverwrite"]):
            if not quiet:
    ...
    def PutNewCol(self, Name, LikeCol="CORRECTED_DATA"):
        if not(Name in self.ColNames):
            print>>log, "  Putting column %s in MS, with format of %s" % (
                Name, LikeCol)
            t = table(self.MSName, readonly=False, ack=False)
            desc = t.getcoldesc(LikeCol)
            desc["name"] = Name
