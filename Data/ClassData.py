...
def testReplace(
        ParsetName="/media/6B5E-87D0/HyperCal2/test/TestDDFacet/ParsetDDFacet.txt"):
...
class ClassGlobalData():
    ...
    def __init__(
        self,
        ParsetFile="/media/tasse/data/HyperCal2/test/ParsetNew.txt",
     ReplaceDico=None):
...
class ClassMultiPointingData():
    ...
    def setMappingBL(self, PointingID=0):
        ...
        if self.DicoConfig["Select"]["FlagAntBL"] != None:
            BLSel = self.DicoConfig["Select"][
                "FlagAntBL"]  # .replace(" ","").split(',')
        else:
            BLSel = []
    ...
    def setCurrentTimeBin(self, xxx_todo_changeme):
        (it0, it1) = xxx_todo_changeme
        self.itimes = it0, it1
        MS = self.giveMS()
