...
#        self.RefFreq=np.mean(self.ListFreqs)
    ...
    def LoadNextVisChunk(self):
        ...
        while True:
            ...
            repLoadChunk = MS.GiveNextChunk(
                databuf=self._databuf, flagbuf=self._flagbuf)
            self.cache = MS.cache
        ...
        if ('F' in DecorrMode) | ("T" in DecorrMode):
            DATA["uvw_dt"] = np.float64(
                self.CurrentMS.Give_dUVW_dt(
                    times, A0, A1))
            DATA["MSInfos"] = np.array(
                [repLoadChunk["dt"],
                 repLoadChunk["dnu"].ravel()[0]],
                np.float32)
        ...
        self.CurrentVisWeights = self.VisWeights[
            self.iCurrentMS][
            self.CurrentMS.current_chunk]
    ...
    def UpdateFlag(self, DATA):
        ...
        if self.DicoSelectOptions["UVRangeKm"] != None:
            ...
            print>>log, "  Flagging uv data outside uv distance of [%5.1f~%5.1f] km" % (
                d0, d1)
            d0 *= 1e3
            d1 *= 1e3
            u, v, w = uvw.T
        ...
        if self.DicoSelectOptions["TimeRange"] != None:
            ...
            print>>log, "  Selecting uv data in time range [%.4f~%5.4f] hours" % (
                st0, st1)
        ...
        if self.DicoSelectOptions["FlagAnts"] != None:
            ...
            if not((FlagAnts is None) | (FlagAnts == "") | (FlagAnts == [])):
                if isinstance(FlagAnts, str):
                    FlagAnts = [FlagAnts]
                for Name in FlagAnts:
                    for iAnt in xrange(MS.na):
                        if Name in MS.StationNames[iAnt]:
                            print>>log, "  Flagging antenna #%2.2i[%s]" % (
                                iAnt, MS.StationNames[iAnt])
        ...
        if self.DicoSelectOptions["DistMaxToCore"] != None:
            ...
            for iAnt in ind.tolist():
                print>>log, "  Flagging antenna #%2.2i[%s] (distance to core: %.1f km)" % (
                    iAnt, MS.StationNames[iAnt], Dist[iAnt]/1e3)
                self.FlagAntNumber.append(iAnt)
    ...
    def CalcWeights(self):
        ...
        WeightMachine.CalcWeights(
            output_list, Robust=Robust, Weighting=self.Weighting,
            Super=self.Super, nbands=self.NFreqBands
            if band_mapping is not None else 1, band_mapping=band_mapping,
            weightnorm=weightnorm, force_unity_weight=weights_are_null)
