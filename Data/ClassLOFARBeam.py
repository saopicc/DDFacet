...
class ClassLOFARBeam():
    ...
    def getBeamSampleTimes(self, times):
        ...
        if not(tmax in TimesBeam):
            TimesBeam.append(tmax)
        return TimesBeam
    ...
    def CalcFreqDomains(self):
        ...
        ChanEdges = np.linspace(
            ChanFreqs.min()-ChanWidth/2.,
            ChanFreqs.max()+ChanWidth/2.,
            NChanJones+1)

        FreqDomains = [[ChanEdges[iF], ChanEdges[iF+1]]
                       for iF in xrange(NChanJones)]
    ...
    def GiveInstrumentBeam(self, *args, **kwargs):
        ...
        for ich in xrange(self.NChanJones):
