...
class ClassBeamMean():
    ...
    def LoadData(self):
        ...
        nr = sum(nrows)
        # preallocate arrays
        # NB: this assumes nchan and ncorr is the same across all MSs in
        # self.ListMS. Tough luck if it isn't!

        print>>log, "Loading some data for all MS..."

        self.Data = {}
        for iMS, MS in zip(xrange(self.VS.nMS), self.ListMS):
    ...
    def CalcMeanBeam(self):
        ...
        for iMS, MS in zip(xrange(self.VS.nMS), self.ListMS):
            JonesMachine = ClassJones.ClassJones(
                self.GD, MS, self.VS.FacetMachine,
                IdSharedMem=self.VS.IdSharedMem)
            ...
            beam_times = np.array(
                JonesMachine.BeamMachine.getBeamSampleTimes(times))
            CurrentBeamITime = -1
            DicoBeam = JonesMachine.EstimateBeam(beam_times, RAs, DECs)

            NTRange = DicoBeam["t0"].size
            for iTRange in xrange(len(DicoBeam)):
                ...
                J0 = J[:, A0s, :, :, :]
                J1 = J[:, A1s, :, :, :]
                JJ = (np.abs(J0[:, :, :, 0, 0])*np.abs(J1[:, :, :, 0, 0]
                                                       )+np.abs(J0[:, :, :, 1, 1])*np.abs(J1[:, :, :, 1, 1]))/2.
                ...
                SumJJsq += SumWsqThisRange.reshape(
                    (self.npix, self.npix, self.MS.Nchan))
    ...
    def Smooth(self):
        ...
        if_z_f_SumJJsq = FT.ifft(
            z_f_SumJJsq.reshape(
                (1, 1, nx, nx))).real.reshape(
            (nx, nx))
