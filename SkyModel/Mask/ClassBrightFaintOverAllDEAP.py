...
log=logger.getLogger("ClassBrightFaintDEAP")
...
class ClassBrightFaintOverAllDEAP():
    ...
    def giveBrightFaintMask(self):
        ...
        CC=SkyModel.Sky.ClassClusterDEAP.ClassCluster(xc,yc,S,
                                                      nNode=45,
                                                      NGen=100,
                                                      NPop=100,
                                                      DoPlot=1,
        ...
        xcc,ycc=xyNodes.reshape((2,nNodes))
        self.xcyc=xcc,ycc

        D=np.sqrt((xc.reshape((-1,1))-xcc.reshape((1,-1)))**2 + (yc.reshape((-1,1))-ycc.reshape((1,-1)))**2)
        mapSourceToNode=np.argmin(D,axis=1)
        iSource=np.arange(xc.size)
        indBrightest=[]
        for iNode in range(nNodes):
            indThisSources=np.where(mapSourceToNode==iNode)[0]
            ThisXc=xc[indThisSources]
            ThisYc=yc[indThisSources]
            ThisS=S[indThisSources]
            ThisiSource=iSource[indThisSources]
            indCal=np.where(ThisS==ThisS.max())[0]
            indBrightest.append(ThisiSource[indCal[0]])
