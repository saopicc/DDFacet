...
class ClassPrint():
    ...
    def Print(self, par, value, value2=None, dest=sys.stdin):
        ...
        if value2 is None:
            ...
            valueOut = "%s%s" % (
                value.ljust(self.WV0),
                (""" "%s" """ % value2).rjust(self.WV1))
    ...
    def Print2(self, par, value, helpit, col="white"):
        ...
        listStrHelp = xrange(0, Lhelp, WidthHelp)

        if listStrHelp[-1] != Lhelp:
            listStrHelp.append(Lhelp)

        print self.proto % (Spar, SFill, Sval)+Shelp[0:WidthHelp]
        for i in xrange(1, len(listStrHelp)-1):
            parout = "%s: %s" % (
                " " * (self.LeftW - 2),
