...
class ClassRestoreMachine():
    ...
    def __init__(self, BaseImageName, BeamPix=5, ResidualImName="", DoAlpha=1,
        ...
        indx, indy = np.int64(np.random.rand(
            Nr)*nx), np.int64(np.random.rand(Nr)*nx)
    ...
    def Restore(self):
        ...
        ListRestoredIm = []
        Lambda = [Lambda0+i*dLambda for i in xrange(self.NBands)]
        ListRestoredImCorr = []
        ListModelIm = []
        ...
        for l in Lambda:
            ...
            print>>log, "Convolve... "
            print>>log, "   MinMax = [%f , %f] @ freq = %f MHz" % (
                ModelImage.min(), ModelImage.max(), freq/1e6)
            RestoredImage = ModFFTW.ConvolveGaussian(
                ModelImage, CellSizeRad=self.CellSizeRad,
        ...
        RestoredImageRes = np.array(ListRestoredIm).reshape(
            (self.NBands, 1, nx, nx))
        RestoredImageResCorr = np.array(
            ListRestoredImCorr).reshape((self.NBands, 1, nx, nx))
        ...
        CasaImage = ClassCasaImage.ClassCasaimage(
            ImageNameModel, RestoredImageRes.shape, self.Cell, self.radec,
            Lambda=(Lambda0, dLambda, self.NBands))
        ...
        CasaImage = ClassCasaImage.ClassCasaimage(
            ImageName, RestoredImageRes.shape, self.Cell, self.radec,
            Lambda=(Lambda0, dLambda, self.NBands))
        ...
        CasaImage = ClassCasaImage.ClassCasaimage(
            ImageNameCorr, RestoredImageResCorr.shape, self.Cell, self.radec,
            Lambda=(Lambda0, dLambda, self.NBands))
        ...
        if self.DoAlpha:
            ...
            print>>log, "  Save... "
            CasaImage = ClassCasaImage.ClassCasaimage(
                ImageName, ModelImage.shape, self.Cell, self.radec)
...
def main(options=None):
    ...
    CRM = ClassRestoreMachine(
        options.BaseImageName,
        BeamPix=options.BeamPix,
        ResidualImName=options.ResidualImage,
