...
def testSetupWithIQUVString():
    conv = ClassStokes(
        [StokesTypes["XX"],
         ...
         StokesTypes["YY"]],
        "IQUV")
    assert conv._MSDataDescriptor == [
        StokesTypes["XX"],
        StokesTypes["XY"],
        StokesTypes["YX"],
        StokesTypes["YY"]]
    assert conv._MScorrLabels == ["XX", "XY", "YX", "YY"]
    assert conv._gridMSCorrMapping == {
        StokesTypes["XX"]: 0,
        StokesTypes["XY"]: 1,
        StokesTypes["YX"]: 2,
     StokesTypes["YY"]: 3}
...
def testSetupWithIQUVString3():
    ...
    assert conv._gridMSCorrMapping == {
        StokesTypes["RL"]: 0, StokesTypes["LR"]: 1}
...
def testCubeTransformCorrs2StokesLinear():
    ...
    corrCube[:, 0, :, 0] = 2
    corrCube[:, 3, :, 0] = 0  # full horizontal (Q = 1)
    corrCube[:, 0, :, 1] = 1
    corrCube[:, 1, :, 1] = 1
    corrCube[:, 2, :, 1] = 1
    corrCube[:, 3, :, 1] = 1  # full 45 linear (U = 1)
    corrCube[:, 0, :, 2] = 1
    corrCube[:, 1, :, 2] = 1j*1
    corrCube[:, 2, :, 2] = -1j*1
    corrCube[:, 3, :, 2] = 1  # full right circular (V = 1)
...
def testCubeTransformCorrs2StokesCircular():
    ...
    corrCube = np.zeros([1, 4, 1, 3], dtype=np.complex64)
    corrCube[:, 0, :, 0] = 2
    corrCube[:, 3, :, 0] = 0  # full right circular (V = 1)
    corrCube[:, 0, :, 1] = 1
    corrCube[:, 1, :, 1] = 1
    corrCube[:, 2, :, 1] = 1
    corrCube[:, 3, :, 1] = 1  # full horizontal (Q = 1)
    corrCube[:, 0, :, 2] = 1
    corrCube[:, 1, :, 2] = 1j * 1
    corrCube[:, 2, :, 2] = -1j * 1
    corrCube[:, 3, :, 2] = 1  # full 45 linear (U = 1)
    stokesCube = conv.corrs2stokes(corrCube)
...
def testCubeTransformStokes2CorrsLinear():
    ...
    stokesCube = np.zeros([1, 4, 1, 3], dtype=np.complex64)
    stokesCube[:, 0, :, 0] = 1
    stokesCube[:, 1, :, 0] = 1  # full horizontal (Q=1)
    stokesCube[:, 0, :, 1] = 1
    stokesCube[:, 2, :, 1] = 1  # full 45 linear (U=1)
    stokesCube[:, 0, :, 2] = 1
    stokesCube[:, 3, :, 2] = 1  # full right circular (V=1)
    corrCube = conv.stokes2corrs(stokesCube)
    assert np.allclose(
        corrCube[0, :, 0, 0],
        np.array([2, 0, 0, 0]))  # I+Q=2 and I-Q=0
    # I+0=1 and U+0=1 and U-0=1 and I-0=0
    assert np.allclose(corrCube[0, :, 0, 1], np.array([1, 1, 1, 1]))
    # I+0=1 and 0+iV=1i and 0-iV=-1i and I-0=1
    assert np.allclose(corrCube[0, :, 0, 2], np.array([1, 1j, -1j, 1]))
...
def testCubeTransformStokes2CorrsLinearNonStandardMSMapping():
    ...
    stokesCube[:, 0, :, 1] = 1
    stokesCube[:, 2, :, 1] = 0.25
    stokesCube[:, 3, :, 1] = 0.75  # 0.25 45 linear (U), 0.75 right circular (V)
    stokesCube[:, 0, :, 2] = 1
    stokesCube[:, 3, :, 2] = 1  # full right circular (V=1)
    corrCube = conv.stokes2corrs(stokesCube)
    assert np.allclose(
    ...
    # I+0=1 and 0+iV=1i and 0-iV=-1i and I-0=1
    assert np.allclose(corrCube[0, :, 0, 2], np.array([1, 1, 1j, -1j]))
...
def testCubeTransformStokes2CorrsCircular():
    ...
    stokesCube[:, 3, :, 2] = 1  # full right circular (V=1)
    corrCube = conv.stokes2corrs(stokesCube)
    assert np.allclose(corrCube[0, :, 0, 0], np.array(
        [1, 1, 1, 1]))  # I+0=1,Q+0=1,Q-0=1,I-0=1
    assert np.allclose(corrCube[0, :, 0, 1], np.array(
        [1, 1j, -1j, 1]))  # I+0=1,0+iU=1i,0-iU=-1i,I-0=1
    assert np.allclose(
        corrCube[0, :, 0, 2],
        np.array([2, 0, 0, 0]))  # I+V=2,I-V=0
