...
def Sphe2D(Npix, factor=1.):
    ...
    for (i, r) in zip(xrange(Npix**2), R.ravel()):
...
def Sphe1D(Npix, factor=1.):
    ...
    for (i, x) in zip(xrange(Npix), xx):
        y[i] = EvalSphe(np.abs(x)*factor)
    return y


def EvalSphe(nu):
    P = np.array([[8.203343e-2, -
                   3.644705e-1, 6.278660e-1, -
                   5.335581e-1, 2.312756e-1], [4.028559e-3, -
                                               3.697768e-2, 1.021332e-1, -
