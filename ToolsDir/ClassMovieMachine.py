...
class MovieMachine():
    ...
    def __init__(self, ParsetFile="ParsetDDFacet.txt",
                 PointingID=0, pngBaseDir="png", TimeCode=(0, -1, 1)):

        MDC, GD = ToolsDir.GiveMDC.GiveMDC(ParsetFile)
        self.MDC = MDC
        self.GD = GD
        self.PointingID = PointingID
        Imager = ClassFacetMachine.ClassFacetMachine(
            MDC, GD, Precision="S", Parallel=True)  # ,Sols=SimulSols)
    ...
    def MakeSnap(self, it0, it1):
        ...
        times, uvw, vis, flag, A0A1 = DicoData['times'], DicoData[
            'uvw'], DicoData['data'], DicoData['flags'], DicoData['A0A1']
    ...
    def MainLoop(self):
        for i in xrange(0, self.NTimes-1, self.Tincr):
