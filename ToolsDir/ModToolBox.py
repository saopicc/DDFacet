...
class FFTM2():
    ...
    def __init__(self, A):
        ...
        #print>>log, "define"
        self.a = pyfftw.n_byte_align_empty(
            A.shape, 16, self.ThisType)  # 'complex128')
        self.b = pyfftw.n_byte_align_empty(
            A.shape, 16, self.ThisType)  # 'complex128')
        #print>>log, "define1"
        self.fft_FORWARD = pyfftw.FFTW(
            self.a, self.b, direction="FFTW_FORWARD", flags=(
                'FFTW_ESTIMATE', ))  # ,threads=4)
        self.fft_BACKWARD = pyfftw.FFTW(
            self.a, self.b, direction="FFTW_BACKWARD", flags=(
                'FFTW_ESTIMATE', ))  # ,threads=4)
        #print>>log, "done"
...
class FFTM2D():
    ...
    def __init__(self, A):
        ...
        #print>>log, "define1"
        self.fft_FORWARD = pyfftw.FFTW(
            self.a, self.b, axes=(-1, -2),
            direction="FFTW_FORWARD", flags=('FFTW_ESTIMATE',))  # ,threads=4)
        self.fft_BACKWARD = pyfftw.FFTW(
            self.a, self.b, axes=(-1, -2),
            direction="FFTW_BACKWARD", flags=('FFTW_ESTIMATE',))  # ,threads=4)
...
def GiveFFTFastSizes(Odd=True, NLim=100000):
    ...
    for base, powers in [
        ...
        sizes = (sizes[np.newaxis, :] * base **
                 np.array(powers)[:, np.newaxis]).ravel()
    sizes = sizes[np.newaxis, :] * np.array([1, 11, 13])[:, np.newaxis]

    # no need to take set(), since sizes are unique by construction (from
    # prime factors...)
