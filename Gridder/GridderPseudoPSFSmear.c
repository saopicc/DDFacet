...
#include "GridderPseudoPSFSmear.h"
#include "complex.h"
#include <omp.h>
...
static PyMethodDef _pyGridderPseudoPSFSmear_testMethods[] = {
	{"pyGridderWPol", pyGridderWPol, METH_VARARGS},
	//	{"pyDeGridderWPol", pyDeGridderWPol, METH_VARARGS},
	{"pyDeGridderWPolPseudoPSF", pyDeGridderWPolPseudoPSF, METH_VARARGS},
...
void init_pyGridderPseudoPSFSmear()  {
  (void) Py_InitModule("_pyGridderPseudoPSFSmear", _pyGridderPseudoPSFSmear_testMethods);
...
void DeGridderWPol(PyArrayObject *grid,
		   ...
		   PyObject *Lmaps, PyObject *LJones, PyArrayObject *SmearMapping, PyObject *LOptimisation)
  {
	...
	for (visChan=chStart; visChan<chEnd; ++visChan) {
	  ...
	  /*   dA_time=sinc...; */
	  /*   for(ThisPol =0; ThisPol<nPolJones;ThisPol++){ */
	  /*     visBuff[ThisPol]*=dA_time*dA_freq; */
	  /*   } */
...
static PyObject *pyDeGridderWPolPseudoPSF(PyObject *self, PyObject *args)
{
  PyObject *ObjGridIn;
  PyObject *ObjVis;
  PyArrayObject *np_grid, *np_vis, *uvw, *cfs, *flags, *sumwt, *increment, *freqs,*WInfos,*SmearMapping;
  ...
  PyObject *Lcfs, *LOptimisation;
  PyObject *Lmaps,*LJones, *ListPseudoPSFCoefs;
  PyObject *LcfsConj;
  int dopsf;
  PyArrayObject*du; PyArrayObject*dv; PyArrayObject*dw; int dt;int df; float freq;float lcor; float mcor;
  if (!PyArg_ParseTuple(args, "O!OO!O!O!iO!O!O!O!O!O!O!O!O!O!O!O!iifff",
			//&ObjGridIn,
			&PyArray_Type,  &np_grid,
			&ObjVis,//&PyArray_Type,  &vis,
			&PyArray_Type,  &uvw,
  ...
  DeGridderWPolPseudoPSF(np_grid, np_vis, uvw, flags, sumwt, dopsf, Lcfs, LcfsConj, WInfos, increment, freqs, Lmaps, LJones, SmearMapping, LOptimisation,du,dv,dw,dt,df,freq,lcor, mcor);
...
void DeGridderWPolPseudoPSF(PyArrayObject *grid,
		   PyArrayObject *vis,
		   PyArrayObject *uvw,
		   PyArrayObject *flags,
		   //PyArrayObject *rows,
		   ...
		   PyArrayObject *increment,
		   PyArrayObject *freqs,
			    PyObject *Lmaps, PyObject *LJones, PyArrayObject *SmearMapping, PyObject *LOptimisation,  PyArrayObject*du,
			    PyArrayObject*dv,PyArrayObject*dw, int dt,int df, float freq, float lcor, float mcor)
  {
    // Get size of convolution functions.
    PyArrayObject *cfs;
    PyArrayObject *NpPolMap;
    ...
    int row0=ptrRows[0];
    int row1=ptrRows[1];
    		////////////// convert the arraylist into a pointer on float32
    //printf("******************herereeeeeeeeeeee*******************************************\n");
    //printf("******************herereeeeeeeeeeee******************************************* *****end\n");
    //int LengthSmearInfo =PyList_Size(ListPseudoPSFCoefs);
    
    
    //double *coefs=p_float64(ListPseudoPSFCoefs);
    //printf("******************herereeeeeeeeeeee*******************************************%f *****end\n",*(p_du+1));
    //double *Plistsmear=p_float64(ListPseudoPSFCoefs);//ListPseudoPSFCoefs->dimensions[0];
    // printf("dimention: %d\n",(size));
    ...
    double *p_dv;
    double *p_dw;
    int dtime; int dfreq; double freq0;double lcordinate; double mcordinate;
   
    if(DoApplySmear>0){
      SmearMode=1;
      //printf("******************no_pseudo PSF *****end\n");
      p_du=p_float64((PyArrayObject *) du);
      p_dv=p_float64((PyArrayObject *) dv);
      p_dw=p_float64((PyArrayObject *) dw);
      dtime=dt; dfreq=df; freq0=freq;lcordinate=lcor; mcordinate=mcor;
      /* while (cpt1<=138){ */
      /* 	printf("******************du=%f, dv=%f, dw=%f\n", *p_du, *p_dv,*p_dw); */
      /* 	p_du++; */
      /* 	p_dv++; */
      /* 	p_dw++; */
      /* 	cpt1++; */
      /* } */
      //      printf("******************no_pseudo PSF%f, %f, %f, dt=%i, df=%i, freq0=%f lcor=%f, mcor=%f*****end\n", *(p_du+2), *(p_dv+2),*(p_dw+2),dtime,dfreq,freq0,lcordinate,lcordinate);

      
      
    //   DoApplySmear=1;
    //  pdu_dt = (PyArrayObject *) PyArray_ContiguousFromObject(PyList_GetItem(ListPseudoPSFCoefs, 0), PyArray_FLOAT64, 0, 54432);
      //float *np_result = p_float32((PyArrayObject *) result);
      //int nrows     = pdu_dt->dimensions[0];
    //  float* p_du_dt=p_float64((PyArrayObject *) pdu_dt);
    //  printf("******************herereeeeeeeeeeee*******************************************%f *****end\n",*(p_du_dt+2));
      //int* ptrRows=I_ptr(NpRows);
      //int row0=ptrRows[0];
      //int row1=ptrRows[1];
    ...
    /* float *list_PseudoPSFcoef = p_float32((PyArrayObject *)ListPseudoPSFCoefs); */
    /* int dt = *(list_PseudoPSFcoef); */
    /* int df = *(list_PseudoPSFcoef+1); */
    /* float * du = *(list_PseudoPSFcoef+2); */
    /* float * dv = *(list_PseudoPSFcoef+3); */
    /* float * uvd = *(list_PseudoPSFcoef+4); */
    /* float freq = *(list_PseudoPSFcoef+5); */
    /* float lcor = *(list_PseudoPSFcoef+6); */
    /* float mcor = *(list_PseudoPSFcoef+7); */
      ...
      if (gridChan >= 0  &&  gridChan < nGridChan) {
      	...
      	if (locx-supx >= 0  &&  locx+supx < nGridX  &&
      	  ...
      	  for (ipol=0; ipol<nVisPol; ++ipol) {
      		...
      		/*   int sx; */
      		/*   for (sx=-fsupx; sx<=fsupx; ++sx) { */
		/*     Delta_time = PI * (*du * lcor + *dv * mcor)*dt; */
		/*     Delta_freq = PI * ((*uvd / freq)*df*lcor + (*uvd / freq)*df*mcor); */

		/*     if (Delta_time==0) Delta_time=1; */
		/*     if (Delta_freq==0) Delta_freq=1; */

      		/*     ThisVis[ipol] += *gridPtr * (sin(Delta_time)/Delta_time)   * (sin(Delta_freq)/Delta_freq)  * *cf0; */
      		/*     cf0 ++; */
      		/*     gridPtr++; */
		/*     du++; dv++; uvd++; */
      		/*   } */
	...
	for (visChan=chStart; visChan<chEnd; ++visChan) {
	  ...
	  double dA_freq=0.;
	  if(SmearMode){
	    /* double *p_du=p_float64((PyArrayObject *) du); */
	    /* double *p_dv=p_float64((PyArrayObject *) dv); */
	    /* double *p_dw=p_float64((PyArrayObject *) dw); */
	    /* dtime=dt; dfreq=df; freq0=freq;lcordinate=lcor; mcordinate=mcor; */
	    du1 = *(p_du);
	    dv1 = *(p_dv);
	    dw1 = *(p_dw);
	    ...
	    float V=(float)uvwPtr[1];
	    float W=(float)uvwPtr[2];
	    float uvd = sqrt(U*U+V*V+W*W);
	    //double l0=ptrFacetInfos[2];
	    //double m0=ptrFacetInfos[3];
	    //double n0=sqrt(1-l0*l0-m0*m0)-1;
	    //printf("****************************DA_time=%f***************\n",dA_time);
	   
	    dA_time=PI*(du1*lcordinate+dv1*mcordinate+dw1*(sqrt(1.-lcordinate*lcordinate-mcordinate*mcordinate)-1.))*dtime;
	    dA_freq=PI*((uvd/freq0)*dfreq*sqrt(lcordinate*lcordinate+mcordinate*mcordinate));//lcordinate + (uvd/freq0)*dfreq*mcordinate);
	    
	    if (dA_time==0){func_time = 1;}
	    else{ func_time = sin(dA_time)/dA_time;}

	    if (dA_freq==0){ func_freq = 1;}
	    else{ func_freq=sin(dA_freq)/dA_freq;}

	    printf("****************************du=%f, dv=%f, dw=%f, lcor=%f, mcor=%f, func_time=%f***************\n", du1,dv1,dw1,lcordinate,mcordinate ,func_time);
	    for(ThisPol =0; ThisPol<nPolVis;ThisPol++){
	      visPtr[ThisPol]*=func_time*func_freq;
	    }
